...
Ltac simpleproof := match goal with
                      | [|- _ <= _] => tryfalse || assumption || lia || trivial || eassumption || auto || idtac
                      | _ => tryfalse || assumption || lia || trivial || reflexivity || eassumption || auto || idtac
                    end.
...
Ltac solve_unsigned_range_lt c x := 
  assert(cge0: c > 0) by lia; 
  assert(zlez: 0 <= 0) by lia;
  assert(xpre: 0 <= x < Int.max_unsigned) by lia;
...
Ltac unsigned_range := 
  match goal with
   | [|- 0 <= ?af] =>
      match af with
        | Z.shiftl _ _ => simpl; try lia
        | Z.shiftr _ _ => simpl; try lia
        | Z.land ?x ?y => apply Z_land_range_lo; try (simpl; lia)
        | Z.lor ?x ?y => apply Z_lor_range_lo; try (simpl; lia)
        | _ => try lia
      end
    | [|- ?af <= Int.max_unsigned] => 
      match af with
        | Z.shiftl _ _ => simpl; try lia
        | Z.shiftr _ _ => simpl; try lia
        | Z.land ?x ?y => apply Z_land_range_hi; try (simpl; lia)
        | Z.lor ?x ?y => apply Z_lor_range_hi; try (simpl; lia)
...
Ltac int_auto_simpl :=
  match goal with
    ...
    | H: context [Int.signed (Int.repr _)] |- _ => rewrite Int.signed_repr in H; [ int_auto_simpl | rewrite min_signed_val, max_signed_val; try lia ]
    | H: context [Int.unsigned (Int.repr _)] |- _ => rewrite Int.unsigned_repr in H; [ int_auto_simpl | rewrite max_unsigned_val; try lia ]
    | |- context [Int.signed (Int.repr _)] => rewrite Int.signed_repr; [ int_auto_simpl | rewrite min_signed_val, max_signed_val; try lia ]
    | |- context [Int.unsigned (Int.repr _)] => rewrite Int.unsigned_repr; [ int_auto_simpl | rewrite max_unsigned_val; try lia ]
...
Ltac stdrange_fin_elem :=
  repeat progress
  match goal with 
      | H : ( _ <= ?a (?b _) <= _)%nat |- _ => apply nat_range.range_l_r_fin_elem in H;[cbv beta delta -[a b] iota in H| unfold nat_has_le.le; lia]
      | H : ( _ <= ?a _ _ <= _)%nat |- _ => apply nat_range.range_l_r_fin_elem in H;[cbv beta delta -[a] iota in H| unfold nat_has_le.le; lia]
      | H : ( _ <= ?a _ <= _)%nat |- _ => apply nat_range.range_l_r_fin_elem in H;[cbv beta delta -[a] iota in H| unfold nat_has_le.le; lia]
      | H : ( _ <= _ <= _)%nat |- _ => apply nat_range.range_l_r_fin_elem in H;[cbv beta delta iota in H| unfold nat_has_le.le; lia]
      | H : ( _ <= ?a (?b _) <= _)%Z |- _ => apply Z_range.range_l_r_fin_elem in H;[cbv beta delta -[a b] iota in H| unfold Z_has_le.le; lia]
      | H : ( _ <= ?a _ _ <= _)%Z |- _ => apply Z_range.range_l_r_fin_elem in H;[cbv beta delta -[a] iota in H| unfold Z_has_le.le; lia]
      | H : ( _ <= ?a _ <= _)%Z |- _ => apply Z_range.range_l_r_fin_elem in H;[cbv beta delta -[a] iota in H| unfold Z_has_le.le; lia]
      | H : ( _ <= _ <= _ )%Z   |- _ => apply Z_range.range_l_r_fin_elem  in H;[cbv beta delta iota in H| unfold Z_has_le.le; lia]
      | _ => fail
  end.
...
Open Scope Z_scope.
  Lemma unsigned_lt2stdrange : forall x b, Int.unsigned x< b -> 0<=Int.unsigned x <= (Z.pred b).
    intros;split;[set (Int.unsigned_range x); tauto| lia].
  Qed.
  ...
  Ltac pre' :=
    match goal with 
       ...
       | H : (?a < ?x < ?b)%nat |- _            => let H' := fresh in rename H into H'; assert ( (S a) <= x <= (pred b))%nat as H by lia; clear H'
       | H : (?a <= ?x < ?b)%nat |- _           => let H' := fresh in rename H into H'; assert ( (a) <= x <= (pred b))%nat as H by lia; clear H'
       | H : (?a < ?x <= ?b)%nat |- _           => let H' := fresh in rename H into H'; assert ( (S a) <= x <= ( b))%nat as H by lia; clear H'
       | H : (?a < ?x < ?b) |- _                => let H' := fresh in rename H into H'; assert ( (1+ a) <= x <= (Z.pred b)) as H by lia; clear H'
       | H : (?a <= ?x < ?b) |- _               => let H' := fresh in rename H into H'; assert ( (a) <= x <= (Z.pred b)) as H by lia; clear H'
       | H : (?a < ?x <= ?b) |- _               => let H' := fresh in rename H into H'; assert ( (1+ a) <= x <= ( b)) as H by lia; clear H'
    end.
  
  Ltac pre := repeat pre';repeat ((* handle even not a real range, 1<=x<=0 for example*)try lia; timeout 8 stdrange_fin_elem).
  Ltac pre_noto := repeat pre';repeat ((* handle even not a real range, 1<=x<=0 for example*)try lia; stdrange_fin_elem).
...
match goal with 
    ...
    | _ => subst; try rewrite max_unsigned_val; solve [ lia | apply div_in_intrange; lia ]
end.
 ...
 *)
  Ltac trivial_ur_rewriter := rewrite Int.unsigned_repr;[idtac| rewrite max_unsigned_val; lia].
...
| _  => subst_all; simpl; repeat trivial_ur_rewriter; simpl; solve [auto| lia |  timeout 2 mycompute; intros; try split; tryfalse; try lia; auto]
end.
  Ltac basic_solver_noto := match goal with 
| H : Int.unsigned ?a = ?b |- _  => rewrite H in *; apply repr_unsigned in H; basic_solver
| _  => subst_all; simpl; repeat trivial_ur_rewriter; simpl; solve [auto| lia |  mycompute; intros; try split; tryfalse; try lia; auto]
end.
...
| _  => subst_all; simpl;  split; try lia
end.
...
Ltac mrewrite := repeat rewrite unsigned_mone_val; repeat rewrite Int.unsigned_repr; try rewrite max_unsigned_val; [idtac|solve [clear; lia |  clear_all_useless_or;  bf_basic_solver] .. ].
Ltac mrewrite_noto := repeat rewrite unsigned_mone_val; repeat rewrite Int.unsigned_repr; try rewrite max_unsigned_val; [idtac|solve [clear; lia |  clear_all_useless_or;  bf_basic_solver_noto] .. ].
...
Qed.
  ...
  Ltac deref_vint32_for_array := match goal with 
                                 | H : nth_val' (Z.to_nat (Int.unsigned ?a) ) OSUnMapVallist = Vint32 _ |- _=> apply OSUnMapVallist_safe_deVint32 in H;[unfold deref_Vint32_unsafe in H; try rewrite H in *; clear H |set (@Int.unsigned_range_2 a); try lia]
                                 | H : nth_val' (Z.to_nat (Int.unsigned ?a) ) OSMapVallist = Vint32 _ |- _ => apply OSMapVallist_safe_deVint32 in H; [unfold deref_Vint32_unsafe in H; try rewrite H in *; clear H | set (@Int.unsigned_range_2 a);  try lia]
                                 | H : nth_val' (Z.to_nat _ ) OSUnMapVallist = Vint32 _ |- _=> apply OSUnMapVallist_safe_deVint32 in H;[unfold deref_Vint32_unsafe in H; try rewrite H in *; clear H | try lia]
                                 | H : nth_val' (Z.to_nat _ ) OSMapVallist = Vint32 _ |- _ => apply OSMapVallist_safe_deVint32 in H; [unfold deref_Vint32_unsafe in H; try rewrite H in *; clear H | try lia]
                                 end.
End _mathsolver_ext.
*)
