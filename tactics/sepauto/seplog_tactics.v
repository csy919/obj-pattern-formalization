...
Proof.
  ...
  assert (length l > n) by lia; clear H.
  apply IHl in H0.
  auto.
Qed.
...
Proof.
  ...
  2 : lia.
  replace (S (length l - S n)) with (length l - n) by lia.
  rewrite IHn.
  2 : lia.
  auto.
Qed.
...
Proof.
  ...
  assert (l = list_dropn l (length l -length l)) 
    as H100 by (replace (length l - length l) with 0 by lia; induction l; auto);
...
Proof.
  ...
  auto.
  simpl in H2;tryfalse.
  simpl in H2; tryfalse. (* added for "eptrconst _" *)
  simpl in H2.
