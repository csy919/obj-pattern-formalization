...
Tactic Notation "math" "simpls" := let H := fresh in math_simpl H; math prove neg H; try solve [bsimpl; lia].
  ...
  match goal with 
    | i : int32 |- _ =>
      ...
      let xrange := fresh in destruct i as [xval xrange]; 
        ...
        solve_int_range'
    | _ => subst; 
        try csimpl Int.max_unsigned; solve [ lia | apply div_in_intrange; lia ]
  end.
...
Ltac trivial_ur_rewriterH H :=
  repeat (rewrite Int.unsigned_repr in H; [idtac |solve [split;[lia| try csimpl Int.max_unsigned; lia]]]).
