...
Proof.
  ...
  (* rearrange H9 and H10 *)
  remember H10; clear Heqo0 H10.
  destruct H4, H9, o0; simpljoin; try solve [ unfold empisr in *; tryfalse ].
...
Lemma excrit1_rule'_ISnil_ISRemp'''_lg:
  forall (Spec : funspec) (r : retasrt) (ri : asrt) 
         (cs : list bool) (frm : asrt) (aop : absop) 
         (p: asrt) ct lg',
    p ==>
      OldOSInvWL ct (logic_val (Vint32 (Int.repr 1))::lg') ** atoy_inv' **
      Aisr empisr **
      Aie false ** Ais nil ** Acs (true :: cs) ** frm ->
    GoodFrm frm ->
    {|Spec, GetHPrio, OSLInv, I, r, ri|}|-ct 
    {{ <|| aop ||>  ** p}}
               (sprim excrit)
    {{  <|| aop ||>  ** Aisr empisr ** Aie true ** Ais nil ** Acs cs ** (p_local OSLInv ct (logic_val (Vint32 (Int.repr 1))::lg') ** frm) }}.
Proof.
  intros.
  eapply excrit1_rule'_ISnil_ISRemp'';eauto.
  intros.
      ...
      match find_aop P with
        | some ?n => 
          hoare lifts (n::nil) pre;
            eapply excrit1_rule'_ISnil_ISRemp'''_lg; 
