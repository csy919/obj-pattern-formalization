...
Definition OS_EventTaskRdy_impl :=
Int8u ·OS_EventTaskRdy·(⌞ptr @ OS_EVENT∗; message @ Void∗ ;  msk @ Int8u⌟)··{
       ⌞
        ptcb @ OS_TCB∗;
        x @ Int8u;
        y @ Int8u;
       ...
       y′ =ₑ OSUnMapTbl′[ptr′→OSEventGrp];ₛ
       bity′ =ₑ OSMapTbl′[y′];ₛ
       x′ =ₑ OSUnMapTbl′[ptr′→OSEventTbl[y′]];ₛ
       bitx′ =ₑ OSMapTbl′[x′];ₛ
       prio′ =ₑ ((y′ ≪ ′3) +ₑ x′);ₛ
       ptr′→OSEventTbl[y′] =ₑ ptr′→OSEventTbl[y′] &ₑ (∼bitx′);ₛ
       If (ptr′→OSEventTbl[y′] ==ₑ ′0) {
           ptr′→OSEventGrp =ₑ ptr′→OSEventGrp &ₑ (∼bity′)
       };ₛ
       ptcb′ =ₑ OSTCBPrioTbl′[prio′];ₛ
       ptcb′→OSTCBDly =ₑ ′0;ₛ
       ptcb′→OSTCBEventPtr =ₑ NULL;ₛ
...
Void ·OS_EventTaskWait·(⌞ptr @ OS_EVENT∗⌟)··{
       ⌞ ⌟;

       OSTCBCur′→OSTCBEventPtr =ₑ ptr′;ₛ
       ...
       ptr′→OSEventTbl[OSTCBCur′→OSTCBY] =ₑ 
               ptr′→OSEventTbl[OSTCBCur′→OSTCBY] |ₑ OSTCBCur′→OSTCBBitX;ₛ
       ptr′→OSEventGrp =ₑ ptr′→OSEventGrp |ₑ OSTCBCur′→OSTCBBitY;ₛ
       RETURN
}·.    

Definition OS_EventTO_impl :=
Void ·OS_EventTO·(⌞ptr @ OS_EVENT∗⌟)··{
       ⌞ ⌟;

       ptr′→OSEventTbl[OSTCBCur′→OSTCBY] =ₑ 
          ptr′→OSEventTbl[OSTCBCur′→OSTCBY] &ₑ (∼OSTCBCur′→OSTCBBitX);ₛ
       If (ptr′→OSEventTbl[OSTCBCur′→OSTCBY] ==ₑ ′0) {
          ptr′→OSEventGrp &=  ∼OSTCBCur′→OSTCBBitY
       };ₛ
...
Void ·OS_InitEventList·(⌞⌟)··{
     ...
     ⌟;
      ...
      ptr1′ =ₑ &ₐ OSEventTbl′[′0];ₛ
      ptr2′ =ₑ &ₐ OSEventTbl′[′1];ₛ
      i′ =ₑ ′0;ₛ
      WHILE (i′ <ₑ (′OS_MAX_EVENTS − ′1)) {
          ptr1′→OSEventType =ₑ ′OS_EVENT_TYPE_UNUSED;ₛ
          ptr1′→OSEventListPtr =ₑ ptr2′;ₛ
          ptr1′→OSEventPtr =ₑ NULL;ₛ
          ++ ptr1′;ₛ
          ++ ptr2′;ₛ
          ++ i′
      };ₛ
...
Void ·OS_EventRemove·(⌞ptr @ OS_EVENT∗⌟)··{
...
Int8u ·OS_EventSearch·(⌞ptr @ OS_EVENT∗⌟)··{
       ⌞p @ OS_EVENT∗; x @ Int8u; p1 @ OS_EVENT∗⌟;
