...
Definition ptr1 : ident := 108%Z.
Definition ptr2 : ident := 109%Z.
...
Definition tasks_post_waiting : ident := 135%Z.
(* constants *)
(* in file 'os_cfg.h' *)
...
Definition kernel_obj_create := 264%Z.
Definition kernel_obj_delete := 265%Z. 
Definition kernel_obj_oper := 262%Z. 
...
Definition errno : ident := 286%Z.

(*defined*)


Definition OS_MAX_EVENTS := 36%Z.
Definition CORENUM := 8%Z.
Definition OS_USER_MAX_EVENTS :=(OS_MAX_EVENTS-CORENUM*2)%Z.
Definition OS_DEL_NO_PEND := 0%Z.
Definition OS_DEL_ALWAYS := 1%Z.


Definition OS_MAX_TASKS:= 63%Z.  (* Definition OS_MAX_TASKS := 30%Z. *) 
(* Definition OS_N_SYS_TASKS := 2%Z.  *)


Definition O_NONBLOCK := 1%Z.

(* Definition OS_EVENT_TYPE_UNUSED := 0%Z. *)

Definition OS_STAT_POSTQ := 64%Z.


(*struct *)

(* Definition obj_arr_ : ident := 700%Z.  *)

Definition service_obj_ : ident := 700%Z.
Definition service_obj : ident := 707%Z. 
Definition obj_arr : ident := 709%Z.

Definition attr : ident := 706%Z. 
  
Definition OSPostEventGrp :ident := 781%Z.
Definition OSPostEventTbl :ident := 782%Z.


(*func name*)
Definition service_obj_create := 288%Z.
Definition service_obj_delete := 296%Z.
Definition service_obj_oper := 282%Z. 

Definition katt := 125%Z.
Definition satt := 127%Z. 

Definition OSQPost := 819%Z.

Definition get_free_obj_idx := 290%Z. 

(* var *)
Definition idx : ident := 285%Z. 
Definition value : ident := 289%Z.
Definition z: ident := 639%Z.
Definition ret: ident := 642%Z.

(** inter func **)
Definition tp : ident := 672%Z.
Definition addr : ident := 670%Z.


(* auxiliary variables in tasks and their values *) 

Definition __OSTskLoc : ident := 788%Z. 

Definition __Loc_normal : Z := 1. 
Definition __Loc_objcre : Z := 2. 
Definition __Loc_objdel : Z := 3.  

Definition __OSTskPtr : ident := 796%Z. 
