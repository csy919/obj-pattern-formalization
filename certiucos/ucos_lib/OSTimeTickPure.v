...
Fixpoint get_ectr (eid:val) (head:val) (ectrl:list EventCtr) :=
  match eid,head,ectrl with
    | (Vptr e),(Vptr e'),(osevent, etbl,petbl)::vll =>
      match beq_addrval e e' with
        | true => Some (osevent,etbl,petbl)
...
Proof.
  ...
  destruct H11.
  assert ($ 1<<ᵢ(x0&ᵢ$ 7) <> $ 0).
  eapply math_prop_neq_zero2; try lia.
  tryfalse.
  split.
  intros.
...
Lemma tick_rdy_ectr_get_ectr_eq:
  forall els vl head l eid,
  tick_rdy_ectr vl head els = Some l ->
 (exists et etbl petbl, get_ectr (Vptr eid) head els = Some (et, etbl,petbl)) ->
 (exists et etbl petbl, get_ectr (Vptr eid) head l = Some (et, etbl,petbl)).
Proof.
  induction els.
  intros.
  unfolds in H.
  ...
  do (* 2 *) 3 eexists;eauto.

  xunfold H2.
  rename HeqH2 into HeqH1; rename HeqH3 into HeqH2. (* new add *)
  ...
  rewrite <- HeqX.
  rewrite <- HeqH3 in *. (* change HeqH2 -> HeqH3 *)
  symmetry in HeqH4. (* change HeqH3 -> HeqH4 *)
  eapply IHels;eauto.
  unfolds.
  instantiate (1:= vl).
  rewrite <- HeqY.
...
Proof.
  ...
  substs.
  simpl in H; mytac.
  unfold TCBNode_P in H6. destruct x2. destruct p. mytac.
  unfolds in H9; mytac.
  unfolds in H11; mytac.
  unfolds in H14.
  unfold WaitTCBblk in H14.
  assert (0 <= Int.unsigned i6 < 64) by auto.
  rename H17 into Hlhq; rename H18 into H17. (* new add *)
...
(* new add *)
Lemma TCBList_P_in_list_get_some_postq :
  forall l v rtbl tcbls i0 i1 i3 i4 i6 i7 i8 i9 i10 a,
    TCBList_P v l rtbl tcbls ->
    In (i0::i1::Vptr a::i3::Vint32 i4::V$OS_STAT_POSTQ::Vint32 i6::i7::i8::i9::i10::nil) l ->
    0 <= Int.unsigned i6 < 64 ->
    array_type_vallist_match Int8u rtbl ->
    length rtbl = ∘OS_RDY_TBL_SIZE ->
    exists tid p n m, TcbMod.get tcbls tid = Some(p, wait(os_stat_postq a) n, m).
Proof.
  inductions l; intros.
  simpl in H0; tryfalse.
  ...
  unfolds in H11; mytac.
  unfolds in H17.
  unfold WaitTCBblk in H17.
  assert (0 <= Int.unsigned i6 < 64) by auto.
  rename H17 into Hlhq; rename H18 into H17.
  pose proof prio_rtbl_dec i6 rtbl H17 H2 H3.
  destruct H18.
  unfolds in H9.
  pose proof H9 i6; clear H9.
  ...
  destruct (get tcbls x); tryfalse.
  substs.
  unfold V_OSTCBPrio in Hlhq.
  unfold V_OSTCBDly in Hlhq.
  unfold V_OSTCBStat in Hlhq.
  unfold V_OSTCBEventPtr in Hlhq.
  simpl in Hlhq.
  assert (exists m0, (p, t, m) = (i6, wait (os_stat_postq a0) i4, m0)).
  apply Hlhq; auto.
  destruct H19; inverts H19.
  eauto.
...
Lemma ECBList_P_get_ectr_some : forall (l:list EventCtr) tl ecbls mqls tcbls a x v,
  ...
  exists et etbl petbl, get_ectr (Vptr a) v l = Some (et, etbl,petbl).
Proof.
  inductions l; intros.
  simpl in H0; mytac.
...
Lemma tcb_eptr_get_ectr':
  forall v'14 v'15 v'3 v'4 v'5 v'6 v'7 v'21 v'28 v'9
    ...
    exists et etbl petbl, get_ectr (Vptr (b, i7)) v'20 v'16 = Some (et,etbl,petbl) .
Proof.
  intros.
  assert
    (i5 = $ OS_STAT_RDY \/
...
(*$ OS_STAT_Q*)
  ...
  apply H11 in H8; mytac; clear H10.
...
(**$OS_STAT_MUTEX*)
  ...
  (*$ OS_STAT_POSTQ*)
  substs.
  assert (exists tid p n m, TcbMod.get v'13 tid = Some (p, wait (os_stat_postq (b, i7)) n, m)).
  apply list_app_in in H2.
  destruct H2.
  assert(exists tid p n m, TcbMod.get v'14 tid = Some (p, wait (os_stat_postq (b, i7)) n, m)).
  eapply TCBList_P_in_list_get_some_postq; eauto.
  mytac.
  pose proof H x.
  rewrite H8 in H10.
  exists x.
  ...
  assert(exists tid p n m, TcbMod.get v'15 tid = Some (p, wait (os_stat_postq (b, i7)) n, m)).
  eapply TCBList_P_in_list_get_some_postq; eauto.
  mytac.
  pose proof H x.
  rewrite H8 in H10.
  ...
  apply H12 in H8; mytac; clear H12. 
...
Lemma tcb_eptr_get_ectr:
  forall v'14 v'15 v'3 v'4 v'5 v'6 v'7 v'21 v'28 v'9
    ...
    0 <= Int.unsigned i4 < 64 -> array_type_vallist_match Int8u v'8 -> length v'8 = ∘OS_RDY_TBL_SIZE ->
    exists et etbl petbl, get_ectr (Vptr (b, i7)) v'20 v'27 = Some (et,etbl,petbl) .
Proof.
  intros.
...
Lemma get_ectr_decompose:
  forall qptrl s P msgqls l x y p eid,
    s |= evsllseg p Vnull qptrl msgqls ** P ->
    get_ectr (Vptr eid) p qptrl = Some (l,x, y) ->
    s |= EX vn qptrl1 qptrl2 msgqls1 msgqls2 msgq, 
  [| V_OSEventListPtr l = Some vn /\
     qptrl = qptrl1 ++ ((l, x,y) :: nil) ++ qptrl2 /\
     msgqls = msgqls1 ++ (msgq :: nil) ++ msgqls2 /\ get_ectr (Vptr eid) p qptrl1 = None /\
   (get_ectr  (Vptr eid) p (qptrl1++ ((l, x,y) :: nil)) = Some (l,x,y)) /\ p <> Vnull /\ 
   forall l,In l qptrl1 -> exists x,V_OSEventListPtr (fst (fst l)) = Some (Vptr x)|] **
                                                     AEventNode (Vptr eid) l x y msgq **
...
Proof.
  ...
  destruct a as (p0,petbl).
  destruct p0 as (osevent,etbl).
(*   destruct a as (osevent,etbl). *)
  ...
  eauto.
  instantiate (1:=x3).
  instantiate (1:= (osevent, etbl,petbl) :: x2).
  simpl.
  auto.
  instantiate (1:= x5).
  instantiate (1:= x6).
...
Lemma tick_rdy_ectr_imp_x:
  forall (v'33 : list (vallist * vallist * vallist)) (v'31 v'10 : val) 
         (v'37 : block) (x6 : val) (i6 i5 i4 i3 i2 i1 i0 i8 i10 : int32)
         (v x5 v'24 : val) (x0 : vallist) (v'34 : list (list val * vallist * vallist))
         (x1 : int32) x xx petbl pegrp petbl_addr,
   (forall l : vallist * vallist * vallist,
    In l v'33 -> exists x2, V_OSEventListPtr (fst (fst l)) = Some (Vptr x2)) ->
   RL_TCBblk_P
     (v'31
      :: v'10
         :: Vptr (v'37, Int.zero)
   ...
   get_ectr (Vptr (v'37, Int.zero)) (Vptr x)
     (v'33 ++
      (xx :: Vint32 i8 :: Vint32 i10 :: v :: x5 :: v'24 :: Vint32 pegrp :: petbl_addr :: nil,
      x0,petbl) :: nil) =
   Some
     (xx :: Vint32 i8 :: Vint32 i10 :: v :: x5 :: v'24 :: Vint32 pegrp :: petbl_addr :: nil,
     x0,petbl) ->
   ...
   Some
     (v'33 ++
      (xx
       :: Vint32 (i8&ᵢInt.not i0) :: Vint32 i10 :: v :: x5 :: v'24 :: Vint32 pegrp :: petbl_addr :: nil,
      update_nth_val (Z.to_nat (Int.unsigned i2)) x0 (Vint32 (x1&ᵢInt.not i1)), petbl)
      :: v'34).
Proof.
  ...
  destruct X;simpl in H5;tryfalse. (* change H4 -> H5 *)
  rewrite H3.
  rewrite H1.
  auto.
  ...
  assert (In (v1, v2, v0) ((v1, v2, v0) :: v'33)) by (simpl;auto).
(*   assert (In (v0,v1) ((v0, v1) :: v'33)) by (simpl;auto). *)
  ...
  assert (tick_rdy_ectr 
       ...
       (v'33 ++
        (xx
         :: Vint32 i8 :: Vint32 i10 :: v :: x5 :: v'24 :: Vint32 pegrp :: petbl_addr :: nil, x0, petbl) :: v'34) = Some
     ( v'33 ++
         (xx
          :: Vint32 (i8&ᵢInt.not i0) :: Vint32 i10 :: v :: x5 :: v'24 :: Vint32 pegrp :: petbl_addr :: nil,
         update_nth_val (Z.to_nat (Int.unsigned i2)) x0
                        (Vint32 (x1&ᵢInt.not i1)), petbl) :: v'34)).
...
Lemma tick_rdy_ectr_imp_x':
  forall (v'33 : list (vallist * vallist * vallist)) (v'31 v'10 : val) 
   ...
   get_ectr (Vptr (v'37, Int.zero)) (Vptr x) v'33 = None ->
   get_ectr (Vptr (v'37, Int.zero)) (Vptr x) 
                     (v'33 ++ (xx :: Vint32 i8 :: Vint32 i10 :: v :: x5 :: v'24 :: Vint32 pegrp :: petbl_addr ::  nil, x0, petbl) ::nil) 
   = Some (xx :: Vint32 i8 :: Vint32 i10 :: v :: x5 :: v'24 :: Vint32 pegrp :: petbl_addr ::  nil, x0,petbl) ->
   tick_rdy_ectr
     (v'31
        :: v'10
        :: Vptr (v'37, Int.zero)
   ...
   Some
     (v'33 ++
           (xx
             :: Vint32 i8 :: Vint32 i10 :: v :: x5 :: v'24 :: Vint32 pegrp :: petbl_addr ::  nil,
            update_nth_val (Z.to_nat (Int.unsigned i2)) x0 (Vint32 (x1&ᵢInt.not i1)), petbl)
           :: v'34).
Proof.
  induction v'33;introv Hin;intros.
...
Lemma tick_rdy_ectr_imp:
  forall v'21,
    isptr v'21 ->
    v'21 <> Vnull ->
    forall v'33 v'31 v'10 v'37 x6 i6 i5 i4 i3 i2 i1 i0 i8 i10 v x5 v'24 x0 v'34 x xx petbl pegrp petbl_addr,
    (forall l,In l v'33 -> exists x, V_OSEventListPtr (fst (fst l)) = Some (Vptr x)) ->
    RL_TCBblk_P
      (v'31
         :: v'10
         :: Vptr (v'37, Int.zero)
    ...
    get_ectr (Vptr (v'37, Int.zero)) v'21 
                      (v'33 ++ (xx:: Vint32 i8 :: Vint32 i10 :: v :: x5 :: v'24 :: Vint32 pegrp :: petbl_addr :: nil, x0,petbl) ::nil) 
    = Some (xx:: Vint32 i8 :: Vint32 i10 :: v :: x5 :: v'24 :: Vint32 pegrp :: petbl_addr :: nil, x0, petbl)->
    tick_rdy_ectr
      (v'31
    ...
    Some
      (v'33 ++
            (xx
              :: Vint32 (i8&ᵢInt.not i0) :: Vint32 i10 :: v :: x5 :: v'24 :: Vint32 pegrp :: petbl_addr :: nil,
             update_nth_val (Z.to_nat (Int.unsigned i2)) x0 (Vint32 (x&ᵢInt.not i1)), petbl)
            :: v'34).
Proof.  
  introv Hv'21 Hv'21'.
  unfolds in Hv'21.
...
Proof.
  ...
  destruct (V_OSEventListPtr v1);auto. (* change v0 -> v1 *)
              ...
              :: msg
                 :: tm
                    :: stat
                       :: Vint32 x
                          :: Vint32 y
                             :: Vint32 z :: Vint32 w :: Vint32 v :: nil) v3 (* change v2 -> v3 *)
...
Definition rel_st_els (st : taskstatus) (els : EcbMod.map) :=
  match st with
    ...
    | (wait (os_stat_postq eid) one) => (* new add *)
      match EcbMod.get els eid with
        | Some (m, wl) => true
        | None => false
      end
...
Definition get_last_ecb_ptr := 
  fun (l : list  (vallist * vallist * vallist)) (x : val) =>
    match l with
      | nil => Some x
      | _ :: _ => V_OSEventListPtr (fst (fst (last l (nil,nil,nil))))
    end.
...
Proof.
  ...
  assert (0=Int.unsigned ($ (Int.unsigned n - 1))) by lia.
...
Proof.
  ...
  auto.
  simpl in H2.
  assert (Has:exists prio', V_OSTCBPrio l' = Some (Vint32 prio') /\ x0 <> prio').
  eapply IHll; eauto.
  mytac.
  rewrite H in H5.
  inverts H5.
...
Proof.
  ...
  eapply  waite_rdy_change with (wl := remove_tid t' wl); try eapply H0; eauto.
  rewrite EcbMod.set_sem.
...
Proof.
  ...
  unfolds RLH_WaitPostQ_P. (* new add *)
  unfolds RHL_Wait_P.
  unfolds RHL_WaitS_P.
  unfolds RHL_WaitQ_P.
  unfolds RHL_WaitMB_P.
  unfolds RHL_WaitMS_P.
  unfolds RHL_WaitPostQ_P. (* new add *)
  unfolds WaitTCBblk.
  ...
  Qed.
   ...
   (* new add *)
  destruct H as ( H & H1 & Hpart1 & Hpart2).
  destruct Hpart2 as (H3 & H4 & Hwait & H6 & H7 & H8 & H9 & H10 & Hrhl_postq).
  destruct Hwait as (H5 & H11 & H12 & H13 & H14 & Hrlh_postq).
  (* end of new add *)
  mytac.
  ...
  eapply V_OSTCBPrio_change_next_ptr'; eauto.
  eapply V_OSTCBDly_change_next_ptr'; eauto.
  assert(V_OSTCBStat vl = Some (V$OS_STAT_POSTQ)).
  eapply V_OSTCBStat_change_next_ptr'; eauto.
  assert(V_OSTCBEventPtr vl = Some (Vptr eid)).
  eapply V_OSTCBEventPtr_change_next_ptr'; eauto.
  eapply Hrlh_postq; eauto.
  (* end of new add *)
  ...
  eapply V_OSTCBStat_change_next_ptr; eauto.
   ...
   (* new add about rhl_postq *)
  intros.
  apply Hrhl_postq in H21; clear - H21 H0.
  mytac; auto.
  eapply V_OSTCBPrio_change_next_ptr; eauto.
  eapply V_OSTCBDly_change_next_ptr; eauto.
  eapply V_OSTCBEventPtr_change_next_ptr; eauto.
...
Proof. 
  ...
  destruct x6.
  simpl in H; mytac; tryfalse.
  simpl in H; mytac; destruct a; destruct p; mytac. (* add destruct p *)
  simpl in H0.
  eapply IHx0; eauto.
  unfolds.
  destruct x0; auto.
...
Proof.
  ...
  simpl in H; mytac; tryfalse.
  simpl in H; mytac; destruct a; destruct p; mytac. (* add destruct p *)
  apply IHl1 in H4. (* change H3 -> H4 *)
  simpl.
  destruct l1.
  simpl.
  simpl in H4; inverts H4; auto. (* change H3 -> H4 *)
  simpl; simpl in H4; auto. (* change H3 -> H4 *)
Qed.
...
Proof.
  ...
  rewrite app_nil_l in H.
  simpl in H; mytac.
  destruct l3; tryfalse; destruct e; destruct p; mytac. (* add destruct p *)
  ...
  destruct l3; tryfalse; mytac.
  destruct e; destruct p; mytac. (* add destruct p *)
  rewrite H0 in H; inverts H.
  clear - H3 H8. (* H2 H6 *)
  unfolds in H3. (* change H2 -> H3 *)
  do 4 eexists; eauto.
  
  rewrite <- app_comm_cons in H.
  unfold ECBList_P in H; fold ECBList_P in H.
  mytac.
  rename H2 into Hrpetbl. rename H3 into H2. (* new add *)
...
Proof.
  ...
  exists (nil:(list EventCtr)) ((v0, v1, v) (* (v, v0) *) :: els) (nil:(list EventData)) (e :: edata) EcbMod.emp hels.
  simpl; mytac; auto.
  apply EcbMod.join_emp; auto.
  assert(EcbMod.get x1 eptr = Some (m, wl)).
  clear - eq1 H3 H0. (* change H2 -> H3 *)
  pose proof tidspec.beq_false_neq _ _ eq1.
  pose proof H3 eptr. (* change H2 -> H3 *)
  rewrite EcbMod.get_sig_none in H1; auto.
  rewrite H0 in H1.
  destruct (EcbMod.get x1 eptr); tryfalse.
  substs; auto.
  
  pose proof IHels edata x2 tail tcbls x1 m wl eptr H5 H6; clear IHels. (* change H4,5 -> H5,6 *)
  mytac.
  exists ((v0, v1, v) (* (v, v0) *) :: x3) x4 (e::x5) x6 (EcbMod.merge (EcbMod.sig x x0) x7) x8.
  mytac; auto.
  simpl; exists x.
  mytac; auto.
  exists x0 x7 x2.
  mytac; auto.
  clear - H3 H11. (* change H2,10 -> H3,11 *)
  unfolds in H3. (* change H2 -> H3 *)
  unfolds.
  
  eapply join_join_merge_1_ecb; eauto.
  clear - H3 H11. (* change H2,10 -> H3,11 *)
  apply EcbMod.join_comm in H3. (* change H2 -> H3 *)
  apply EcbMod.join_comm in H11. (* change H10 -> H11 *)
...
Proof.
  ...
  simpl in H; mytac; tryfalse.
  simpl in H; mytac.
  rewrite H3 in H2; inverts H2. (* change H1,2 -> H2,3 *)
  destruct l3; tryfalse.
  destruct e; destruct p; mytac. (* add destruct p *)
  intro.
  simpl in H10; destruct H10; tryfalse.  (* change H8 -> H10 *)
  inverts H10.
  pose proof joinsig_joinsig_neq_ecb H4 H2; tryfalse. (* change H1,3 -> H2,4 *)
  ...
  destruct p; destruct p0. (* new add *)
...
Lemma ecblist_p_compose':
  forall  p qid mqls qptrl1 qptrl2 i i1 a x3 p' v'41 
          msgqls1 msgqls2 msgq mqls1 mqls2 mq mqls' tcbls x petbl pegrp petbl_addr,
    R_ECB_ETbl_P qid
                 (x
                    :: Vint32 i ::  i1 ::  a :: x3 :: p' :: Vint32 pegrp :: petbl_addr :: nil, v'41, petbl) tcbls ->
    R_ECB_PETbl_P qid
                 (x
                    :: Vint32 i ::  i1 ::  a :: x3 :: p' :: Vint32 pegrp :: petbl_addr :: nil, v'41, petbl) tcbls -> (* new add *)
    ...
    ECBList_P p Vnull (qptrl1 ++ ((x
                                     :: Vint32 i :: i1 :: a :: x3 :: p' :: Vint32 pegrp :: petbl_addr :: nil, v'41, petbl)::nil) ++ qptrl2)
                                           (msgqls1 ++ (msgq::nil) ++msgqls2) mqls tcbls.
Proof.
  intros.
  simpl.
...
Lemma R_ECB_PETbl_P_hold_for_set_rdy:
forall tcbls tid prio msg msg' x0 v0 v1 v st,
~(exists eid nl, st = wait (os_stat_postq eid) nl ) ->
get tcbls tid = Some (prio, st, msg) ->
R_ECB_PETbl_P x0 (v0, v1, v) tcbls ->
R_ECB_PETbl_P x0 (v0, v1, v) (set tcbls tid (prio, rdy, msg')).
Proof.
intros.
funfold H1.
unfolds.
...
mytac.
assert (Hc: x = tid \/ x <> tid) by tauto.
destruct Hc.
subst.
unfold Maps.get in H4.
unfold TcbMap in H4.
...
Proof.
  ...
  (* new add about R_ECB_PETbl_P *)
  eapply R_ECB_PETbl_P_hold_for_set_rdy; eauto.
  introv Hf. destruct Hf as (ed & wt &Hf).
  inverts Hf.
  (* end of add *)
...
Proof.
  ...
  simpl in H0.
  mytac.
  rename H2 into Hpetbl; rename H3 into H2. (* new add *)
  simpl.
  eexists;splits;eauto.
  2: {  destruct edata;auto;destruct a;destruct p0; tryfalse.
           mytac.
         eapply R_ECB_PETbl_P_hold_for_set_rdy; eauto.
         introv Hf. destruct Hf as (ed & wt &Hf).
         inverts Hf.
  ...
  Focus 2.
  destruct edata;auto;destruct a;destruct p0; eauto. (* add destruct p0*)
  mytac.
  do 3 eexists.
  mytac;eauto.
...
Proof.
  ...
  Lemma r_ecb_petbl_p_tcbstats_eq_hold:
    forall x x0 a htls (st:tcbstats) n m n' m' p,
      TcbMod.get htls x = Some (p, wait st n, m) ->
      R_ECB_PETbl_P x0 a htls ->
      R_ECB_PETbl_P x0 a (set htls x (p, wait st n', m')).
  Proof.
  intros.
    unfolds;unfolds in H0.
    destruct a.
    ...
    mytac.
    assert (Hc: x1 = x \/ x1 <> x) by tauto.
    destruct Hc.
    subst.
    unfold Maps.get in H2.
    unfold TcbMap in H2.
  ...
  eapply r_ecb_petbl_p_tcbstats_eq_hold;eauto. (* new add *)
...
Proof.
  ...
  destruct b.
  do 2 eexists.
  eapply waite_rdy_change with (x:= os_stat_postq e); eauto.
  do 2 eexists.
  eapply waite_change; eauto.
  (* end of new add *)
  pose proof int_dec_zero_one i.
...
Proof.
  ...
  rewrite H; auto.
  unfolds in H0; mytac.
  apply H5 in H; mytac. (* change H4 -> H5 *)
  unfold Maps.get in *;simpl in *.
  rewrite H; auto.
  (* new add *)
  unfolds in H0; mytac.
...
(*4*)
  assert(x = os_stat_sem eid \/ x = os_stat_q eid \/
         x = os_stat_mbox eid \/ x = os_stat_mutexsem eid \/ x = os_stat_postq eid).
  tauto.
  ...
  do 3 (destruct H2; substs; inversion H0); tryfalse. (* add tryfalse *)
  rewrite TcbMod.set_a_get_a'; auto.
  eauto.
...
(*4*)
  assert(x = os_stat_mbox eid \/ x = os_stat_q eid \/
         x = os_stat_sem eid \/ x = os_stat_mutexsem eid \/ x = os_stat_postq eid).
  tauto.
  clear H3; rename H2 into H3.
  destruct H3; substs.
  unfolds in H; mytac.
...
Lemma RH_TCBList_ECBList_MUTEX_OWNER_P_hold_ecb':
forall ecbls tcbls eid st wl wl',
EcbMod.get ecbls eid = Some (st,wl) ->
RH_TCBList_ECBList_MUTEX_OWNER ecbls tcbls ->
RH_TCBList_ECBList_MUTEX_OWNER_P ecbls tcbls ->
RH_TCBList_ECBList_MUTEX_OWNER_P (EcbMod.set ecbls eid (st,wl')) tcbls.
Proof.
introv H How. intros.
funfold H0.
unfolds.
splits; intros;
try rewrite EcbMod.set_sem in *;
try destruct (tidspec.beq eid eid0) eqn: eq1;
try destruct (tidspec.beq eid eid1) eqn: eq1;
...
eapply H1; eauto.
eapply H1; eauto.
destruct (tidspec.beq eid1 eid2) eqn: eq2.
eapply tidspec.beq_true_eq in eq2.
subst. inverts H4.
tryfalse.
eapply H2; eauto.
destruct (tidspec.beq eid eid2) eqn: eq2.
eapply tidspec.beq_true_eq in eq2.
subst. inverts H4.
eapply H2; eauto.
eapply H2; eauto.
...
(*2*)
  ...
  eapply OSTimeDlyPure.RH_TCBList_ECBList_MUTEX_OWNER_P_hold_tcb; eauto. (* new add *)
(*3*)
  unfolds in H; mytac.
  rename H3 into Hown.
  unfolds; splits; intros.
...
(*4*) 
  assert(x = os_stat_mutexsem eid \/ x = os_stat_q eid \/
         x = os_stat_sem eid \/ x = os_stat_mbox eid  \/ x = os_stat_postq eid).
  tauto.
  clear H3; rename H2 into H3.
  destruct H3; substs.
  unfolds in H; mytac.
  ...
  apply H4 in H0; auto.
  rewrite TcbMod.set_a_get_a' in H3; auto.
  apply H4 in H3; eauto.
  eapply rh_tcblist_ecblist_mutex_owner_set;eauto.
  eapply OSTimeDlyPure.RH_TCBList_ECBList_MUTEX_OWNER_P_hold_tcb; eauto. (* new add *)
  unfolds in H; mytac.
  unfolds; splits; intros.
  apply H in H8; mytac. (* change H7 -> H8 *)
  destruct (tidspec.beq t tid) eqn : eq1.
  pose proof tidspec.beq_true_eq _ _ eq1; substs.
  unfold Maps.get in *;simpl in *.
  rewrite H8 in H0; inverts H0. (* change H7 -> H8 *)
  destruct H2; tryfalse; destruct H0; tryfalse.
  destruct H0; tryfalse. (* new add *)
  rewrite TcbMod.set_a_get_a'; eauto.
  ...
  destruct (tidspec.beq t tid) eqn : eq1.
  pose proof tidspec.beq_true_eq _ _ eq1; substs.
  rewrite TcbMod.set_a_get_a in H8; auto; tryfalse. (* change H7 -> H8 *)
  inverts H8. (* change H7 -> H8 *)
  destruct H2; tryfalse; destruct H2; tryfalse.
  destruct H2; tryfalse. (* new add *)
  rewrite TcbMod.set_a_get_a' in H8; auto. (* change H7 -> H8 *)
  apply H5 in H8; eauto. (* change H7 -> H8 *)
  eapply rh_tcblist_ecblist_mutex_owner_set;eauto.
  eapply OSTimeDlyPure.RH_TCBList_ECBList_MUTEX_OWNER_P_hold_tcb; eauto. (* new add *)
  (*5*)
  assert(x = os_stat_mutexsem eid \/ x = os_stat_q eid \/
  ...
  apply in_remove_tid_false in H7; tryfalse. (* change H6 -> H7 *)
  rewrite TcbMod.set_a_get_a'; auto.

  pose proof in_remove_tid H7. (* change H6 -> H7 *)
  ...
  eapply rh_tcblist_ecblist_mutex_owner_set';eauto.
  eapply OSTimeDlyPure.RH_TCBList_ECBList_MUTEX_OWNER_P_hold_tcb; eauto. (* new add *)
  eapply RH_TCBList_ECBList_MUTEX_OWNER_P_hold_ecb'; eauto. (* new add *)
Qed.
...
(*2*)
  unfolds in H; mytac.
  rename H2 into Hpostq1; rename H5 into Hpostq2; (* new add *)
  rename H3 into H2; rename H4 into H3. (* new add *)
  unfolds; splits; intros.
  apply H in H4; mytac.
  destruct (tidspec.beq t tid) eqn : eq1.
  pose proof tidspec.beq_true_eq _ _ eq1; substs.
...
(*   apply H2 in H4; eauto. *)
  ...
  pose proof tidspec.beq_true_eq _ _ eq1; substs.
   rewrite TcbMod.set_a_get_a in H4; auto; tryfalse.
  rewrite TcbMod.set_a_get_a' in H4; auto.
  eapply Hpostq2; eauto.
  (* end of new add *)
(*3*)
  unfolds in H; mytac.
  rename H2 into Hpostq1; rename H4 into Hpostq2; (* new add *)
  rename H3 into H2. (* new add *)
  unfolds; splits; intros.
  apply H in H3; mytac.
  destruct (tidspec.beq t tid) eqn : eq1.
  pose proof tidspec.beq_true_eq _ _ eq1; substs.
...
(*4*)
  destruct H3; substs.
  unfolds in H; mytac.
  rename H4 into Hpostq1; rename H6 into Hpostq2; (* new add *)
  rename H5 into H4. (* new add *)
  unfolds; splits; intros.
  apply H in H5; mytac.
  destruct (tidspec.beq t tid) eqn : eq1.
  pose proof tidspec.beq_true_eq _ _ eq1; substs.
  ...
  destruct (tidspec.beq t tid) eqn : eq1.
  pose proof tidspec.beq_true_eq _ _ eq1; substs.
   rewrite TcbMod.set_a_get_a in H5; auto; tryfalse.
  rewrite TcbMod.set_a_get_a' in H5; auto.
  eapply Hpostq2; eauto.
  (* end of new add *)

  unfolds in H; mytac.
  rename H5 into Hpostq1; rename H7 into Hpostq2; (* new add *)
  rename H6 into H5. (* new add *)
  unfolds; splits; intros.
  apply H in H6; mytac.
  destruct (tidspec.beq t tid) eqn : eq1.
  pose proof tidspec.beq_true_eq _ _ eq1; substs.
 ...
 (* new add about postq *)
  ...
  (*5*)
  unfolds in H; mytac.
  rename H3 into Hpostq1; rename H6 into Hpostq2; (* new add *)
  rename H4 into H3; rename H5 into H4. (* new add *)
  unfolds; splits.
  intros.
  destruct (tidspec.beq eid eid0) eqn : eq1.
  pose proof tidspec.beq_true_eq _ _ eq1; substs.
  rewrite EcbMod.set_a_get_a in H5; mytac.
  inverts H5.
  destruct (tidspec.beq t tid) eqn : eq2.
  pose proof tidspec.beq_true_eq _ _ eq2; substs.
  ...
  pose proof in_remove_tid H6.
  assert(EcbMod.get els eid0 = Some (absmsgq x0 y pqwaitset, wl)/\In tid wl) by auto.
  apply H in H7; mytac.
  eauto.
  auto.
  ...
  inverts H0.
  apply tidspec.beq_false_neq in eq1; tryfalse.
  do 3 (destruct H2; substs; inversion H0); tryfalse.
  rewrite TcbMod.set_a_get_a'; auto.
  eauto.
  ...
  introv Hget.
  destruct (tidspec.beq eid eid0) eqn : eq1.
  pose proof tidspec.beq_true_eq _ _ eq1; substs; auto.
  rewrite EcbMod.set_a_get_a in Hget; mytac.
  inverts H5.
  assert (exists prio m n,
          Maps.get tls tid = Some (prio, wait (os_stat_postq eid0) n, m)).
  { eapply Hpostq1; eauto. }
  mytac.
  destruct (tidspec.beq t tid) eqn : eq2.
  pose proof tidspec.beq_true_eq _ _ eq2; substs; auto.
  unfold Maps.get in *;simpl in *.
  rewrite H5 in H0; inverts H0.
  destruct H2; tryfalse.
  do 3 (destruct H0; tryfalse).
  inverts H0.
...
Proof.
  ...
  clear H2.
  rename Hres into H2.
  splits;intros;destruct H2 as (H0 & H2 & How & Howp). (* change H2 -> H2 as (...) *)
  assert (t<>tid).
  intro.
  subst.
  eapply H0 in H.
...
(*   destruct H0;auto. *)
  ...
  clear Hx.
  splits;intros;destruct H2 as (H0 & H1 & How & Howp). (* change H2 -> H2 as (...) *)
  assert (t= tid \/ t <>tid) by tauto.
  assert  (eid=eid0 \/ eid<>eid0) by tauto.
  destruct H2;subst.
  destruct H3;subst.
...
Proof.
  ...
  inverts H1.
  splits; try unfolds ; intros.
  destruct H6 as (_&_ &Hw &_).
  destruct Hw as (Hw &_).
  lets Hx : Hw H1 H4.
  unfolds in H1.
  destruct H1 as (_&_& Hv).
  usimpl Hv.
  destruct Hx.
  apply int_ltu_eq_false in H1.
  false.
  destruct H6 as (_&_ &Hw &_).
  destruct Hw as (_&Hw &_).
  lets Hx : Hw H1 H4 H7.
