...
Lemma semacc_eventtype_neq_sem:
  forall s P p ectrl1 a b c ectrl2 msgqls1 msgq msgqls2 mqls tcbls  qid mqls1 mqls' mq mqls2 t,
    s |= AEventData a msgq ** P ->
    RLH_ECBData_P msgq mq ->
    length ectrl1 = length msgqls1 ->
    ECBList_P p Vnull 
              (ectrl1 ++ (((a,b),c)::nil) ++ ectrl2)
              (msgqls1 ++ (msgq::nil) ++ msgqls2)
              mqls tcbls ->
    ECBList_P p (Vptr qid) ectrl1 msgqls1 mqls1 tcbls ->
    EcbMod.join mqls1 mqls' mqls ->
...
Lemma semacc_compose_EcbList_P:
  forall p qid a b c tcbls i n x2 x3 vn msgq mq ectrl1 msgqls1 mqls1 ectrl2 msgqls2 mqls2 mqls' mqls pegrp petbl_addr,
    R_ECB_ETbl_P qid ((a,b),c) tcbls ->
    R_ECB_PETbl_P qid ((a,b),c) tcbls ->
    a = (V$OS_EVENT_TYPE_SEM :: Vint32 i :: Vint32 n :: x2 :: x3 :: vn :: Vint32 pegrp :: petbl_addr :: nil) ->
    RLH_ECBData_P msgq mq ->
    ECBList_P p (Vptr qid) ectrl1 msgqls1 mqls1 tcbls ->
    ECBList_P vn Vnull ectrl2 msgqls2 mqls2 tcbls ->
    EcbMod.joinsig qid mq mqls2 mqls' ->
    EcbMod.join mqls1 mqls' mqls ->
    ECBList_P p Vnull (ectrl1 ++ (((a,b),c)::nil) ++ ectrl2) 
              (msgqls1 ++ (msgq::nil) ++ msgqls2)
              mqls tcbls.
  intros.
  subst.
  ...
  forall mqls tcbls ct sid ecbls p l i v1,
    ...
    ECBList_P (Vptr sid) Vnull
              ((V$OS_EVENT_TYPE_SEM
                ...
                INIT_EVENT_TBL, INIT_EVENT_PTBL) :: l) (DSem i :: ecbls)
...
Proof.
  ...
  unfolds.
  intros.
  destruct Ha1 as (Hab & Had & Hac & Hae).
  lets Hre : Hac H.
