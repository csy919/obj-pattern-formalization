...
Lemma prio_available_hold_same_prio:
forall tls pip tid p st m st' m',
TcbMod.get tls tid = Some (p, st, m) ->
prio_available pip tls ->
prio_available pip (TcbMod.set tls tid (p, st', m')).
Proof.
unfold prio_available.
intros.
rewrite TcbMod.set_sem in *.
destruct (tidspec.beq tid t) eqn: eq1;
eapply H0; eauto.
inverts H1.
eauto.
Qed.

Lemma prio_available_hold_gen:
forall tls pip tid p st m,
Int.eq pip p = false ->
prio_available pip tls ->
prio_available pip (TcbMod.set tls tid (p, st, m)).
Proof.
...
Lemma RH_TCBList_ECBList_MUTEX_OWNER_P_hold_tcb:
forall ecbls tcbls tid prio st m st' m',
TcbMod.get tcbls tid = Some (prio, st, m) ->
RH_TCBList_ECBList_MUTEX_OWNER_P ecbls tcbls ->
RH_TCBList_ECBList_MUTEX_OWNER_P ecbls (TcbMod.set tcbls tid (prio, st', m')).
Proof.
intros.
funfold H0.
unfolds.
splits; intros.
eapply prio_available_hold_same_prio; eauto.
eapply prio_available_hold_same_prio; eauto.
rewrite TcbMod.set_sem in *.
destruct (tidspec.beq tid tid0) eqn: eq1.
eapply tidspec.beq_true_eq in eq1.
subst. inverts H4.
eapply H1; eauto.
eapply H1; eauto.
eapply H2;eauto.
...
forall ecbls tcbls eid st wl,
~ (exists pip o, st = absmutexsem pip o) ->
RH_TCBList_ECBList_MUTEX_OWNER_P ecbls tcbls ->
...
Lemma ecb_petbl_prop:
  forall st  b prio msg x v v0 tcbls i,
    st = rdy \/ (exists n, st = wait os_stat_time n) ->
    TcbMod.get tcbls (b, Int.zero) = Some (prio, st, msg) ->
    R_ECB_PETbl_P x (v, v0) tcbls ->
    R_ECB_PETbl_P x (v, v0)
                 (TcbMod.set tcbls (b, Int.zero)
                             (prio, wait os_stat_time i, msg)).
Proof.
  destruct v.
...
Proof.
  ...
  split.
  eapply ecb_petbl_prop; eauto. (*add*)
  exists x0.
  eexists.
  exists x2.
  splits; eauto.
...
Proof.
  ...
  unfolds in H3.
  unfolds in H4.
  rename H6 into H6'. (* new add *)
  unfolds in H6'. (* new add *)
  unfold RdyTCBblk in *.
