...
(* modified due to introduction of petbl *)
Definition ecb_get_next_ptr (ecb: EventCtr) :=
  let (a, b) := ecb in
  let (vl, _) := a in
  V_OSEventListPtr vl.
...
Proof.
  ...
  destruct a; destruct p (* added *); sep pure.
...
Lemma AEventNode_vnull_false :
  forall s v v0 v1 e0 P,
    s |= AEventNode Vnull v v0 v1 e0 ** P ->
    False.
Proof.
  intros.
...
*)
  ...
  unfold evsllseg in H; fold evsllseg in H; destruct e1; destruct p (* added *); sep pure.
...
Proof.
  ...
  destruct l1.
  unfolds in H0; simpl in H0. destruct a; destruct p (* added*). unfolds in H0; simpl in H0.
  simpl in H.
  destruct H.
...
Lemma evsllseg_append1 :
 forall (v'9:list EventCtr) (v'11:list EventData) v'6 v'13 v'12 x4 (v1:val) x6 v'8 x8 v'10 v0 v1 v'9 v'11 s d i0 
      lpetbl petbl pegrp,
    RL_Tbl_Grp_P v0 v'12 ->
    id_addrval' (Vptr (v'13, Int.zero)) OSEventTbl os_ucos_h.OS_EVENT = Some v'10 ->
    struct_type_vallist_match os_ucos_h.OS_EVENT
  ...
  s |= evsllseg v'6 (Vptr (v'13, Int.zero)) v'9 v'11 **
       ...
       AOSEventTbl v'10 v0 ** AOSPostEventTbl lpetbl petbl ** AEventData (i0 :: v'12 :: x4 :: v1 :: x6 :: v'8 :: x8) d ->
   id_addrval' (Vptr (v'13, Int.zero)) OSPostEventTbl os_ucos_h.OS_EVENT =
            Some lpetbl ->
    V_OSPostEventGrp (i0 :: v'12 :: x4 :: v1 :: x6 :: v'8 :: x8) = Some pegrp ->
    RL_Tbl_Grp_P petbl pegrp ->
  s |= evsllseg v'6 v'8 (v'9 ++ (i0 :: v'12 :: x4 :: v1 :: x6 :: v'8 :: x8, v0, petbl) :: nil) (v'11 ++ d :: nil).
Proof.
  intros.
  gen v'6 v'13 v'12 x4 x6 v'8 x8 v'10 v0 v2. gen v'1 s d i0 lpetbl petbl pegrp. clear - v'0.
...
Proof.
  ...
  unfold evsllseg in H; fold evsllseg in H.
  destruct e; destruct p (* added*); unfold AEventNode in H; unfold AOSEvent in H; unfold node in H.
...
Proof.
  ...
  destruct (V_OSEventListPtr v0) eqn : eqx.
  destruct (val_beq (Vptr x) v2) eqn : eq1; auto.

  assert (get_last_ptr (e :: l) = Some (Vptr x)).
  unfolds; auto.
  pose proof IHl x v2 h t H0; clear IHl.
...
Proof.
  ...
  destruct e; destruct p (* added *); auto.
  ...
  destruct e0; destruct p (* added *); destruct (V_OSEventListPtr v0).
  destruct (val_beq p1 v2); tryfalse.
  unfolds; auto.
  unfolds; auto.
  ...
  destruct (V_OSEventListPtr v0) eqn : eq2.
  destruct (val_beq p1 v2) eqn : eq3; tryfalse.
...
Proof.
  ...
  unfold evsllseg in H; fold evsllseg in H.
  destruct e; destruct p0 (*added*); sep pure.
  simpl.
  split.
  intros.
  rewrite H0 in H1; inverts H1.
  ...
  sep lift 5%nat in H. (* 4->5 due to addition of AOSPostEventTbl  *)
  eapply Astruct_osevent_dup_false'; eauto. 
  simpl in H; simpljoin1; tryfalse.
  ...
  simpl in H; simpljoin1; tryfalse.
  unfold evsllseg in H; fold evsllseg in H.
  destruct a; destruct p0 (*added*); sep pure.
  simpl.
  split.
  intros.
  rewrite H0 in H1; inverts H1.
  ...
  sep lift 6%nat in H. (* 5->6 due to addition of AOSPostEventTbl *)
  eapply Astruct_osevent_dup_false'; eauto.
  ...
  simpl in H; simpljoin1; tryfalse.
  unfold evsllseg in H; fold evsllseg in H.
  destruct e1; destruct p0 (*added*); sep pure.
  sep lift 4%nat in H. 
  eapply AEventNode_dup_false; eauto.
...
Lemma upd_last_ectrls_last : forall l v1 v2 v3 v4 v5 v6 v7 etbl petbl x,
  upd_last_ectrls (l ++ ((v1::v2::v3::v4::v5::v6::v7), etbl, petbl)::nil) x =
  l ++ ((v1::v2::v3::v4::v5::x::v7), etbl, petbl)::nil.                             
Proof.
  inductions l; intros.
  simpl; auto.
  do 2 rewrite <- app_comm_cons.
  ...
  simpl length.
  simpl.
  assert (p :: l ++ (v1 :: v2 :: v3 :: v4 :: v5 :: x :: v7, etbl, petbl) :: nil =
          (p :: l) ++ (v1 :: v2 :: v3 :: v4 :: v5 :: x :: v7, etbl, petbl) :: nil). 
...
Qed.
  ...
  pose proof IHl v1 v2 v3 v4 v5 v6 v7 etbl petbl x; clear IHl.
  rewrite <- app_comm_cons in H0.
