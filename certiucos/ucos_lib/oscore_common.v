...
Proof.
  ...
  apply Hr12 in Hl1.
  lets Hl2 : math_and_shf_ltu_true Hpy. lia.
  apply Hr22 in Hl2.
  clear Hr12 Hr22.
  clear H7.
  ...
  mauto.
  tryfalse.
  lets Hrszz :  math_highest_prio_select Hnth3 Hnth1 Hz Hnty;eauto; try lia.
  eapply nat_8_range_conver; eauto.
  ...
  clear -Hnq H7.
  assert (Int.unsigned ((x<<ᵢ$ 3)+ᵢy) < Int.unsigned prio' \/
          Int.unsigned ((x<<ᵢ$ 3)+ᵢy) = Int.unsigned prio') by lia.
  destruct H; auto.
  clear H7.
  false.
  apply Hnq.
...
Proof.
  ...
  sep normal in H.
  sep destruct H.
  sep lift 10%nat in H.
  apply disj_split in H.
  destruct H.
  sep normal in H.
  sep destruct H.
  sep split in H.
  sep lifts (3::24::nil)%nat in H.
  destruct H0.
  subst x6.
...
Lemma task_del_noexists:
  forall v'9 v'7 v'13 ct v'21 v'10 v'11 P xx xxx logicl',
    dllseg v'7 xx v'13 (Vptr ct) v'9 OS_TCB_flag V_OSTCBPrev V_OSTCBNext **
      ...
      V_OSTCBNext **
      dllsegflag v'7 xxx (v'9 ++ v'10 :: v'11) V_OSTCBNext **
      LINV OSLInv ct (logic_val (V$ 0) :: logicl') ** P 
      ==> Afalse.
Proof.
  inductions v'9.
  intros.
...
Lemma inv_change_aux':
  forall p1 p2 tcbl1 tcbcur tcbl2 rtbl tcbls ptfree lfree P t lg',
    AOSTCBList' p1 p2 tcbl1 (tcbcur :: tcbl2) rtbl t tcbls ptfree **
                AOSTCBFreeList' ptfree lfree t  tcbls** p_local OSLInv t (logic_val (V$1) :: lg') ** P <==>
                AOSTCBList_old p1 p2 tcbl1 (tcbcur :: tcbl2) rtbl t tcbls **
                AOSTCBFreeList ptfree lfree ** tcbdllflag p1 (tcbl1 ++ tcbcur :: tcbl2) ** LINV OSLInv t (logic_val (V$1) :: lg') ** P.
Proof.
  intros.
  split;intros.
  unfold AOSTCBList' in H.
...
(****)
  ...
  unfold AOSTCBFreeList in H.
  (* sep lift 5%nat. *)
  sep_lift_context AOSTCBFreeList'.
  unfold AOSTCBFreeList'.
  (* sep lift 2%nat. *)
...
Lemma inv_change_aux:
  forall p1 p2 tcbl1 tcbcur tcbl2 rtbl tcbls ptfree lfree P t lg',
    AOSTCBList' p1 p2 tcbl1 (tcbcur :: tcbl2) rtbl t tcbls ptfree **
                ...
                AOSTCBFreeList ptfree lfree ** tcbdllflag p1 (tcbl1 ++ tcbcur :: tcbl2) ** p_local OSLInv t (logic_val (V$ 1) :: lg') ** P.
Proof.
  intros.
  split;intros.
