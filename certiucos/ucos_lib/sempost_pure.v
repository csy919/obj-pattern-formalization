...
Lemma sempost_inc_cnt_prop:
  forall s P a msgq mq a' msgq' mq' n i x2 x3 x4 qid b tcbls petbl pegrp petbl_addr,
    s |= AEventData a msgq ** P ->
    RLH_ECBData_P msgq mq ->
    R_ECB_ETbl_P qid (a,b,petbl) tcbls ->
    a = (V$OS_EVENT_TYPE_SEM :: Vint32 i :: Vint32 n :: x2 :: x3 :: x4 :: Vint32 pegrp :: petbl_addr :: nil) ->
    msgq = DSem n ->
    mq = (abssem n, nil) ->
    a' = (V$OS_EVENT_TYPE_SEM :: Vint32 i :: Vint32 (n+ᵢ$ 1)  :: x2 :: x3 :: x4 :: Vint32 pegrp :: petbl_addr :: nil) ->
    msgq' = DSem (n+ᵢ$ 1) ->
    mq' = (abssem (n+ᵢ$ 1), nil) ->
    Int.ltu n ($ 65535) = true ->
    s |= AEventData a' msgq' **
         [| RLH_ECBData_P msgq' mq' |] ** 
         [| R_ECB_ETbl_P qid (a',b,petbl) tcbls |] ** P. 
Proof.
  intros.
  sep pauto.
...
Lemma sempost_grp_wls_nil:
  forall (v'36 v'13 : vallist) (v'12 : int32) 
         ...
         (vhold : addrval) petbl pegrp petbl_addr,
    v'12 = Int.zero ->
    ...
    R_ECB_ETbl_P (v'32, Int.zero)
                 (V$OS_EVENT_TYPE_SEM
                   :: Vint32 v'12
                   :: Vint32 x :: v'24 :: v'35 :: v'0 :: Vint32 pegrp :: petbl_addr :: nil,
                  v'13,petbl) v'7 ->
...
Lemma post_exwt_succ_pre_sem
     : forall (v'36 v'13 : vallist) (v'12 : int32) 
       ...
       R_ECB_ETbl_P (v'32, Int.zero)
         (V$OS_EVENT_TYPE_SEM
          :: Vint32 v'12
             :: Vint32 x :: Vptr (v'24, Int.zero) :: v'35 :: v'0 :: Vint32 pegrp :: petbl_addr :: nil,
         v'13,petbl) v'7 ->
       RH_TCBList_ECBList_P v'11 v'7 v'8 ->
       EcbMod.join v'9 v'10 v'11 ->
       EcbMod.joinsig (v'32, Int.zero) (abssem x , x1) v'6 v'10 ->
...
: forall (p : int32) (etbl : vallist) (ptbl : list val) 
         ...
         (qid : addrval) (vle : list val) (vhold : addrval) petbl,
    0 <= Int.unsigned p < 64 ->
    ...
    R_ECB_ETbl_P qid (V$OS_EVENT_TYPE_SEM :: vle, etbl,petbl) tcbls ->
...
(* copy form OSMutexPendPure_ext.v *)
...
destruct Hc.
subst.
unfold Maps.get in H4,H0.
unfold TcbMap in H4,H0.
...
Proof.
  ...
  introv Hf. destruct Hf as (qd & qwt & Hf). inverts Hf.
...
: forall (v'36 : vallist) (v'12 : int32) (v'13 : vallist)
         ...
         (msg0 msg1: msg) (y : int32) (vhold : addrval) petbl pegrp petbl_addr,
              ...
              (v'30 ++
                    ((V$OS_EVENT_TYPE_SEM
                      ...
                      update_nth_val (Z.to_nat (Int.unsigned v'38)) v'13
                                     (Vint32 (v'69&ᵢInt.not v'40)),petbl) :: nil) ++ v'1)
              (v'31 ++
                    (DSem v'15 ::nil)
                    ++ v'5)
...
Proof.
  ...
  unfold V_OSEventType; simpl. pure_auto.
...
Proof.
  ...
  destruct H as (H&_).
  destruct H.
  destruct H6 as (Hpostq1 & H6 & Hpostq2). (* new add *)
  assert (eid = eid0 \/ eid <> eid0) by tauto.
  destruct H7;subst.
  rewrite EcbMod.set_sem in H4.
  rewrite tidspec.eq_beq_true in H4; auto.
  ...
  rewrite EcbMod.set_sem in H4.
  rewrite tidspec.neq_beq_false in H4; auto.
  assert (EcbMod.get v'11 eid0 = Some (absmsgq x2 y pqwaitset, qwaitset) /\ In tid0 qwaitset).
  split;auto.
  apply H in H8.
  mytac.
  assert (tid = tid0 \/ tid <> tid0) by tauto.
  ...
  destruct H as (H&_).
  destruct H.
  destruct H5 as (Hpostq1 & H5 & Hpostq2). (* new add *)
  assert (tid = tid0 \/ tid <> tid0) by tauto.
  destruct H6;subst.
  rewrite TcbMod.set_sem in H4.
  rewrite tidspec.eq_beq_true in H4; auto.
  ...
  destruct H as (_ & _ & _ & Hrb).
  eapply RH_TCBList_ECBList_MUTEX_OWNER_P_hold_ecb;
  try eapply OSTimeDlyPure.RH_TCBList_ECBList_MUTEX_OWNER_P_hold_tcb; eauto.
  pure_auto.
  do 2 destruct H.
  inverts H.
Qed.
...
Lemma sempost_grp_wls_nil':
  forall v'36 v'6 vhold v'7 v'13 v'12 v'32 x v'24 v'35 v'0 v'11 v'8 v'9 v'10 x1 petbl pegrp petbl_addr,
    v'12 = Int.zero ->
    R_PrioTbl_P v'36 v'7 vhold ->
    RL_Tbl_Grp_P v'13 (Vint32 v'12) ->
    R_ECB_ETbl_P (v'32, Int.zero)
...
(***************** Q begin ************)
  ...
  destruct Hq as [F1 [Fpostq1 [F2 Fpostq2]]].
  intuition.
  ...
  match goal with
    | H: TcbMod.get _ _ = _ |- _ =>
        apply Fpostq2 in H; mytac; tryfalse
  end.
      
  rewrite EcbMod.set_a_get_a'; 
    [ eapply Fpostq2; eauto
    | auto].
  (* end of new add *)
...
(**************** Q end ************)  
  ...
  destruct Hmutex as [F1 [F2 [Fow Fowp]]].
  intuition.
  destruct (tidspec.beq a eid) eqn:Feq.
  apply tidspec.beq_true_eq in Feq; substs.
  unfold get in *; simpl in *.
  ...
  match goal with
    | Hget: TcbMod.get _ _ = _ |- _ =>
        apply F2 in Hget; mytac; tryfalse
  end.
  ...
  introv Hf. destruct Hf as (fp & fo & Hf). tryfalse.
Qed.
...
Lemma sempost_struct_type_vallist_match_sem:
  forall i1 x2 x3 v'44 pegrp petbl_addr,
    isptr v'44 ->
    isptr x2 ->
    Int.ltu i1 ($ 65535) = true ->
    Int.unsigned pegrp <= 255 -> (* new add *)
    struct_type_vallist_match OS_EVENT
                              (V$OS_EVENT_TYPE_SEM
                                :: V$0 :: Vint32 (i1+ᵢ$ 1) :: x2 :: x3 :: v'44 :: Vint32 pegrp :: petbl_addr :: nil).
Proof.
  intros.
...
Lemma rl_etbl_ptbl_p:
  forall l egrp v'33 i4 x6 v'22 etbl tcbls ptbl etype av petbl pegrp petbl_addr,
    array_type_vallist_match Int8u etbl ->
    ...
    R_ECB_ETbl_P l
                 (etype
                   :: Vint32 egrp
                   :: Vint32 i4 :: v'33 
                   :: x6 :: v'22 :: Vint32 pegrp :: petbl_addr ::  nil,
                  etbl,petbl) tcbls ->
