  ...
  apply backward_rule1 with (p:= (
    (EX v ectrl1 ectrl2 msgql1 msgql2,
     <|| aop ||> **
         ...
         LV ptr @ os_ucos_h.OS_EVENT ∗ |-> Vptr v'7
         ** H4
    )
     \\//
    (EX a ectrl1 ectrl2 msgqls1 msgqls2 osevent etbl petbl msgq,
     <|| aop ||> **
         LV x @ Int8u |-> (V$1) **
         ...
         AEventNode (Vptr a) osevent etbl petbl msgq **
         [|V_OSEventListPtr osevent = Some (Vptr v'7)|] **
         evsllseg (Vptr v'7) Vnull ectrl2 msgqls2 **
         [|ectrl1 ++ (osevent, etbl, petbl)::ectrl2 = ((e::v'4) ++ e0 :: v'5) /\
...
(*prove v'10 and v'12 are not nill, so we can unfold the mqsllseg for one node*)
  ...
  unfold AOSEvent.
  unfold node.
  destruct x0 as (p & petbl).
  destruct p.
  hoare normal pre.
  hoare_ex_intro.
  rename v'13 into lpetbl.
  rename v'14 into pegrp.
  rename v'15 into v'13.
  seplog_pattern_tacs.hoare_lifts_trms_pre (OSPostEventTbl, V_OSPostEventGrp, RL_Tbl_Grp_P petbl).
  eapply pure_split_rule'; introv Hlpetbl.
  eapply pure_split_rule'; introv Hpegrp.
  eapply pure_split_rule'; introv Hpetbl.
  hoare_split_pure_all.
  destruct H3.
  assert(exists v1 v2 v3 v4 v5 v6 tail, v = v1::v2::v3::v4::v5::v6::tail).
  apply struct_type_vallist_match_os_event; auto.
...
(*1: the case of x=0*)  
  left.
  exists v'8 (v'9 ++ (x0 :: v'12 :: x5 :: x6 :: x7 :: v'8 :: x9, v0,petbl)::nil) x1.
  exists (v'11 ++ x2 :: nil) x3.
  sep pauto.
  sep cancel 1%nat 1%nat.
  do 5 sep cancel 6%nat 2%nat.
  sep lift 5%nat in H3.
...
(*proved*)
  ...
  rename v'15 into petbl; rename v'16 into v'15.
  destruct e3 as (p & petbl').
  destruct p.
  unfold AOSEvent.
  unfold node.
  hoare normal pre.
  eapply ex_intro_rule; intro.
  eapply ex_intro_rule; intro.
  eapply ex_intro_rule; intro lpetbl.
  eapply ex_intro_rule; intro pegrp.
  hoare_ex_intro.
  rename v'22 into lpetbl'.
  rename v'23 into pegrp'.
  rename v'24 into v'22.
  seplog_pattern_tacs.hoare_lifts_trms_pre (OSPostEventTbl, V_OSPostEventGrp, RL_Tbl_Grp_P petbl').
  eapply pure_split_rule'; introv Hlpetbl'.
  eapply pure_split_rule'; introv Hpegrp'.
  eapply pure_split_rule'; introv Hpetbl'.
...
(*L13: p->OSEventListPtr = p1->OSEventListPtr;*)
  ...
  hoare_change 15%nat (AEventData (x :: x0 :: x1 :: x2 :: x3 :: x11 :: x5) v'15).
...
assert ((v'9 ++ (x :: x0 :: x1 :: x2 :: x3 :: Vptr (v'22, Int.zero) :: x5, v'14,petbl) :: nil) = (e :: v'4)).
  pose proof ecbls_ptr_distinct_get_last_eq (v'9 ++ (x :: x0 :: x1 :: x2 :: x3 :: Vptr(v'22, Int.zero) :: x5, v'14,petbl) :: nil) ((x6 :: x7 :: x8 :: x9 :: x10 :: v'19 :: x12, v0,petbl') :: v'10) (e::v'4) (e0::v'5) (Vptr (v'22, Int.zero)).
  rewrite <- H8 in Hx.
  apply H6; auto.
...
(*length v'11 = length v'9, by H6: length v'9 + 1 = length v'4 + 1,
  ...
  sep lift 15%nat in H21.
  apply evsllseg_list_len_eq in H21; auto.
  
  assert (length (v'9 ++
       (x :: x0 :: x1 :: x2 :: x3 :: Vptr (v'22, Int.zero) :: x5, v'14,petbl)
  ...
  exists (x6 :: x7 :: x8 :: x9 :: x10 :: v'19 :: x12, v0,petbl') e2.
  exists (x6 :: x7 :: x8 :: x9 :: x10 :: v'19 :: x12) v0 petbl'.
  ...
  sep cancel AOSPostEventTbl.
(*   sep cancel Aier. *)
  do 4 sep cancel 10%nat 3%nat.
  sep cancel A_dom_lenv.
  sep cancel (Astruct (v'22, Int.zero)).
  sep cancel (AOSEventTbl v'20).
  sep cancel Aop.
  ...
  assert (s |= evsllseg v'6 v'19 (v'9++(x :: v'17 :: x1 :: x2 :: x3 :: v'19 :: x5, v'14,petbl) :: nil) (v'11 ++ v'15 :: nil) **
                       evsllseg v'19 Vnull v'5 v'3 **
                       p_local OSLInv tid (logic_val (V$ 1) :: lg')).
  sep cancel p_local.
  sep cancel 1%nat 2%nat.
  sep lifts (5::2::3::4::1::nil)%nat in H21.
  eapply evsllseg_append1; eauto.
