...
Ltac hoare_exit_critical2 :=
  ...
  1: hoare_exit_critical2.
  3: first [ intros s H; exact H | sep pauto ] .
  remember (A_dom_lenv
              ((pevent, OS_EVENT ∗)
...
Ltac hoare_call2 :=
  ...
  simpl getasrt.
  unfold OS_SchedPost'.
(* Here part of Ltac fails *)
Ltac sep_semiauto2 :=
    intros;
    match goal with
      | H:?s |= _
        |- ?s |= _ =>
        try (solve [ exact H | apply atrue_intro ]); sep normal in H; sep destruct H; sep split in H; subst(*;
  ...
  hoare normal pre; hoare_ex_intro; eapply forward_rule2.
  1: eapply rete_rule'; [ symbolic execution | sep_semiauto2 ].
  2: try sep_unfold_funpost; sep pauto.
  sep normal.
  do 5 eexists.
  sep split in H116.
