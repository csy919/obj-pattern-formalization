...
| absinfer_prim_abt':
  forall p q step vl sc li t,
    can_change_aop p ->
    can_change_aop q ->
    absimplication sc li (<||step (|vl|)||> ** p) (<||ABORT||> ** q) t 
    -> absinferfull sc li (<||step (|vl|)||> ** p) (<||ABORT||> ** q) t 
...
| absinfer_seq_abt': forall p q s sc li t,
    can_change_aop p ->
    can_change_aop q ->
    p ==> q ->
    absinferfull sc li (<||ABORT ;; s||> **p) (<||ABORT||> ** q) t
...
|absinfer_assert_cont' : 
   forall  p (b:absexpr) q sc li t,
     can_change_aop p ->
     can_change_aop q ->
     p  ==> q ->
     (forall s, (s |= p) -> b (getabst s)) ->     
     absinferfull sc li (<||ASSERT b||> ** p) (<||END None||> ** q) t
...
(*      can_change_aop q -> *)
(*      p  ==> q -> *)
(*      (exists s, (s |= p) /\ ~b (getabst s)) ->   *)
(*      absinferfull sc li (<||ASSERT b||> ** p) (<||ABORT||> ** q) t    *)
...
(* | absabort_rule :  forall Spec sd I r ri p p' p'' q s pa t, *)
(*     #sd,pa,t ⊢ p⇒p' -> p' = <|| ABORT ||> ** p'' ->  *)
(*     InfRules Spec sd pa I r ri p s q t *)
| absabort_rule : forall Spec sd I r ri p q s pa t,
        ...
        match dl_vl_match d1 (rev vl) with
          | true =>
            match buildp (dladd d1 d2) vl with
              | Some p2 =>
                Some   
                  ( (((EX vl', <|| (fst abs) ((rev vl) ++ vl') ||>) ** p_local pa t lg ** p2 **Aie true ** Ais nil ** Acs nil ** Aisr empisr)
                       ** A_dom_lenv (getlenvdom (revlcons d1 d2))))
              | None => None
            end
          |false => None
...
| absinfer_abt:
    forall p q step vl sc, 
      can_change_aop p ->
      can_change_aop q ->
      absimp sc (<||step (|vl|)||> ** p) (<||ABORT||> ** q)
      -> absinfer sc (<||step (|vl|)||> ** p) (<||ABORT||> ** q)  
...
| absinfer_seq_abt: forall p q s sc,
    can_change_aop p ->
    can_change_aop q ->
    p ==> q ->
    absinfer sc (<|| ABORT;; s ||> ** p) (<|| ABORT ||> ** q) 
...
|absinfer_assert_cont : 
   forall  p (b:absexpr) q sc,
     ...
     p  ==> q ->
     (forall s, (s |= p) -> b (getabst s)) ->     
     absinfer sc (<||ASSERT b||> ** p) (<||END None||> ** q) 
...
(*      p  ==> q -> *)
(*      (exists s, (s |= p) /\ ~b (getabst s)) ->   *)
(*      absinfer sc (<||ASSERT b||> ** p) (<||ABORT||> ** q)  *)
...
Proof.
  ...
  eapply absinfer_prim_abt'; eauto. eapply absimp_imp_full; eauto.

  eapply  absinfer_seq'; eauto.
  eapply absinfer_seq_end'; eauto.

  eapply absinfer_seq_abt'; eauto. 

  eapply absinfer_choice1'; eauto.
  eapply absinfer_choice2'; eauto.
  eapply absinfer_assume' ; eauto.

  eapply absinfer_assert_cont'; eauto.
  eapply absinfer_assert_abt'; eauto.
