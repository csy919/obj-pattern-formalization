...
Definition EventCtr:= (prod (prod vallist vallist) vallist).
...
(* new precise ; the old is renamed into precise'*)
...
Lemma precise'_is_precise : forall p,
 precise' p -> precise p.
Proof.
 unfold precise', precise;intros.
 pose proof H s M1 M2 o1 o2 H0 H1. destruct H6.
 split.
 eapply H6;eauto.
 eapply H7;eauto.
Qed.
...
(*add by wss*)
Fixpoint Aarray_halfpermission' (l:addrval) (n :nat) t vl :=
  match vl, n with
    | nil, 0%nat => Aemp
    | v :: vl', S n' => 
      let (b,i) := l in
      PV l @ t |-r-> v ** Aarray_halfpermission' (b, (Int.add i (Int.repr (Z_of_nat (typelen t))))) n' t vl'
    | _,  _ => Afalse
  end.

Definition Aarray_half l t vl :=
  match t with
    | Tarray t' n => Aarray_halfpermission' l n t' vl 
    | _ => Afalse
  end.

Definition GAarray_half l x t vl := 
  G& x @ t == l ** Aarray_half l t vl.
(*add end*)
...
(* Aarray half remove m *)
Fixpoint Aarray_half_rm (l:addrval) (n:nat) (t:type) (vl:list val) (m:nat) {struct vl}:=
  match l with 
    | (b,i) =>
      match vl with
        | nil => Aemp
        | cons v vl' => match n with
                          ...
                          | S n' => 
                            match m with
                              | 0 => Aarray_halfpermission' (b, Int.add i (Int.repr (BinInt.Z.of_nat (typelen t)))) n' t vl'
                              | S m' => PV l @ t |-r-> v ** Aarray_half_rm (b, Int.add i (Int.repr (BinInt.Z.of_nat (typelen t)))) n' t vl' m'
                            end
                        end
      end
  end.
...
Fixpoint Aarray_struct' (l:addrval) m dls1 dls2 vl {struct vl}:=
  match vl, m with
    | nil,0 => Aemp
    | v :: vl', S m' =>
      match dls2 with 
      | dnil => 
        ...
        | dcons id' t' dls' => 
          let (b,i):=l in 
          PV l @ t' |-> v ** 
          Aarray_struct' (b,(Int.add i (Int.repr (Z_of_nat (typelen t'))))) m' dls1 dls1 vl'
        end
      | dcons id' t' dls' => 
        let (b,i):=l in 
        PV l @ t' |-> v ** 
        Aarray_struct' (b,(Int.add i (Int.repr (Z_of_nat (typelen t'))))) m' dls1 dls' vl'
      end
   | _,_ => Afalse
  end.

Definition len_of_struct t len := 
exists vl, struct_type_vallist_match t vl /\ len = length vl.

Definition Aarray_struct l n t vl :=
  match t with
  | Tstruct id dls => 
      (EX len, Aarray_struct' l (n * len) dls dls vl ** [| len_of_struct t len |])
  | _ => Afalse
  end.
(* not the typelen, but the elements number of struct *)

Fixpoint Aarray_new' (l:addrval) m t (vll:list vallist) :=
  match vll, m with
    | nil,0 => Aemp
    | vl1 :: vll', S m' =>
      match t with
      | Tstruct _ _ =>
         let (b,i):=l in 
         Astruct l t vl1 ** Aarray_new' (b,(Int.add i (Int.repr (Z_of_nat (typelen t))))) m' t vll'
      | Tarray _ _ =>
         let (b,i):=l in 
         Aarray l t vl1 ** Aarray_new' (b,(Int.add i (Int.repr (Z_of_nat (typelen t))))) m' t vll'
      | _ => 
          match vl1 with
          | v :: nil =>
             let (b,i):=l in 
             PV l @ t |-> v ** Aarray_new' (b,(Int.add i (Int.repr (Z_of_nat (typelen t))))) m' t vll'
         | _ => Afalse
          end
    end
  | _,_ => Afalse
...
Definition Aarray_new l t vll :=
  match t with
    | Tarray t' n => Aarray_new' l n t' vll 
    | _ => Afalse
  end.

Fixpoint new_array_type_vallist_match t (vll:list vallist) :=
  match vll with
  | nil => True
  | vl1 :: vll' =>
      match t with
      |  Tstruct _ _ =>
            struct_type_vallist_match t vl1 /\ new_array_type_vallist_match t vll'
      | Tarray t' n =>
            array_type_vallist_match t' vl1 /\ new_array_type_vallist_match t vll'
      | _ => 
            array_type_vallist_match t vl1 /\ new_array_type_vallist_match t vll'
      end
 end.
...
(* add by wss*)
Fixpoint Astruct_half' (l : addrval) dls vl {struct dls}:=
  match vl, dls with
    | nil,dnil => Aemp
    | v :: vl', dcons id t dls' => 
        match t with 
        | Tarray t' n => let (b,i):=l in Astruct_half' (b, (Int.add i (Int.repr (Z_of_nat (typelen t))))) dls' vl'
        | Tstruct _ _ => let (b,i):=l in Astruct_half' (b, (Int.add i (Int.repr (Z_of_nat (typelen t))))) dls' vl' 
        | _ => let (b,i):=l in
               PV l @ t |-r-> v ** Astruct_half' (b, (Int.add i (Int.repr (Z_of_nat (typelen t))))) dls' vl'
        end
    | _,  _ => Afalse
  end.

Definition Astruct_half l t vl :=
  match t with
    | Tstruct id dls => Astruct_half' l dls vl 
    | _ => Afalse
  end.

Definition node_half (v:val) (vl:vallist) (t:type) :=
  EX b, [| v= Vptr (b,Int.zero) /\ struct_type_vallist_match t vl |]
          ** Astruct_half (b,Int.zero) t vl.
Fixpoint dllseg_half (head headprev tail tailnext : val) (l:list vallist) 
         (t:type) (pre next: vallist -> option val):=
  match l with
    | nil => [| head = tailnext |] ** [| headprev = tail |]
    | vl::l' => [| head <> Vnull |] ** EX vn,
                [|next vl = Some vn|] ** [|pre vl = Some headprev|] **
                node_half head vl t ** dllseg_half vn head tail tailnext l' t pre next
  end.
(*add end*)
