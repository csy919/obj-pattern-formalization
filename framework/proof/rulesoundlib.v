...
Proof.
  ...
  inductions ks; inductions ks'; tryfalse; simpl in H; try inverts H; try auto;simpl in H0 ; try lia;
  try (do 4 decEq;eapply IHks;eauto;try lia).
Qed.
...
Proof.
  ...
  mytac.
  eauto.
  assert (gettopis is + 0 + 1 =gettopis is + 1)%nat by lia.
  eapply inv_isr_irrev_prop ; eauto.
  right.
  splits;mytac; eauto.
Qed.
...
(* kwhile false *)
  ...
  trivial.
  trivial. trivial.
  trivial. (* added after introduction of "eptrconst ..." *) 
Qed.
...
Proof.
  ...
  introv Hsat1  Hjm2 Hj Hlst Hnabt.
  ...
  intros. (* added due to the abstract non-abortion condition *) 
...
Proof.
  ...
  introv Hinv Hjm Hjon Hstep Hnabt. 
  invertstep idtac.
  ...
  introv Hinv Hjm Hdisj Hjoin Hnabt Hstep.  
  unfold joinm2 in Hjm.
  unfold joinmem in Hjm.
  simp join.
  simpl in Hinv.
...
Proof.
  ...
  introv Hfcal Hinv Hjm1  Hjon Hlostep Hnabt. 
  ...
  introv Hnotm Hts Hts' Hdisj Hjoin Hnabt.
  unfolds in Hts'.
  simp join.
...
Proof.
  ...
  introv  Hjj2 Hinv Hdisj Hjoin Hnabt Hff.
  apply Hff.
  exists (c',(ke',ks)) o2.         
...
Proof.
  ...
  introv Hsat.
  trysimplsall.
  assert (S i = S (i+0)) by lia.
  rewrite <- H.
  auto.
  introv Hsat.
  trysimplsall.
  assert (S (i + S j) = S (S i + j)) by lia.
...
Proof.                                            
  ...
  auto.
  introv Hsat.
  replace ((S (i + S j))) with (S (S i +  j)) by lia.
  apply IHj.
  apply starinv_prop2.
  auto.
Qed.
...
Proof.
  ...
  destruct H;mytac.
  simpl in H;simpl in H1; mytac.
  replace ((S (S (i + j)))) with (S (i + S j)) by lia.
  eapply IHi;eauto.
  destruct H; mytac.
  simpl in H; simpl in H1; mytac.
...
Lemma  starinv_isr_prop_intro2 : forall I i j,   
  ...
  trysimplsall.
  sep cancel 1%nat 1%nat.
  replace (j+1)%nat with (S j) in Hsat by lia.
  apply starinv_prop1 in Hsat.
  sep auto.
  introv Hsat.
  replace (S j + S (S i))%nat with (S (S j) +  (S i))%nat in Hsat by lia.
  apply IHi in Hsat.
...
Lemma  starinv_isr_prop_rev :(forall I i j,    
  ...
  sep auto.
  introv Hsat.
  replace (S j + S (S i))%nat with (S (S j) +  (S i))%nat  by lia.
  apply IHi.
...
Lemma starinv_isr_precise :
  forall n low I s M1 M2 o1 o2,
    ...
    (forall (M : mem) (o : osabst), Maps.sub M1 M -> Maps.sub M2 M -> 
      Maps.sub o1 o -> Maps.sub o2 o -> M1 = M2 /\ o1 = o2).
Proof.
  inductions n; intros.
  destruct s as [[[[[[]]]]]].
  simpl substmo in *.
  ...
  all : unfold precise in p;
  pose proof p (e, e0, M1, x0, l, o1, a) M1 M2 o1 o2;
  simpl substmo in H;
  pose proof H H19 H9;
  specialize (H0 _ _ H1 H2 H3 H4);tauto. } 
  ...
  unfold emposabst in *.
  simpljoin.
  destruct H13, H8; mytac; tryfalse.

  1,2 : pose proof IHn (S low) I (e, e0, M1, x11, l, o1, a) M1 M2 o1 o2;
  simpl substmo in H5;
  pose proof H5 H14 H9;
  specialize (H6 _ _ H1 H2 H3 H4); tauto.

  remember (I low) as X; destruct X; destruct prec.
  simpl assertion.getinv in H15, H25.
  unfold precise in p.
  pose proof p (e, e0, x5, x11, l, x8, a) x5 x x8 x2.
  simpl substmo in H.
  pose proof H H25 H15.
  assert (Maps.sub x5 M1). { eapply join_sub_l; eauto. }
  assert (Maps.sub x5 M). { eapply sub_trans; eauto. }
  assert (Maps.sub x M2). { eapply join_sub_l; eauto. }
  assert (Maps.sub x M). { eapply sub_trans; eauto. }
  assert (Maps.sub x8 o1). { eapply join_sub_l; eauto. }
  assert (Maps.sub x8 o). { eapply sub_trans; eauto. }
  assert (Maps.sub x2 o2). { eapply join_sub_l; eauto. }
  assert (Maps.sub x2 o). { eapply sub_trans; eauto. }
  destruct (H0 M o H8 H16 H18 H20) as [HM HO]. subst.
  pose proof IHn (S low) I (e, e0, x6, x11, l, x9, a) x6 x0 x9 x3 H14 H9.
  assert (Maps.sub x6 M1). { eapply join_sub_r; eauto. }
  assert (Maps.sub x6 M). { eapply sub_trans; eauto. }
  assert (Maps.sub x0 M2). { eapply join_sub_r; eauto. }
  assert (Maps.sub x0 M). { eapply sub_trans; eauto. }
  assert (Maps.sub x9 o1). { eapply join_sub_r; eauto. }
  assert (Maps.sub x9 o). { eapply sub_trans; eauto. }
  assert (Maps.sub x3 o2). { eapply join_sub_r; eauto. }
  assert (Maps.sub x3 o). { eapply sub_trans; eauto. }
  destruct (H21 M o H23 H27 H29 H31) as [HM' HO']. subst.
  jeat. 
...
Proof.
  ...
  simpl empst in *.
  mytac.
  destruct H28, H14; mytac; tryfalse.
  unfold invlth_isr in *.
  pose proof starinv_isr_precise (INUM - (gettopis i1 + 1)) (gettopis i1 + 1) I
       (e, e0, M1, i, (false, i1, c), o1, a) M1 M2 o1 o2 H16 H9.
  eapply H; eauto.

  rewrite H0 in H10.
  clear - H10.
  apply Nat.lt_irrefl in H10; false.

  rewrite H in H8.
  apply Nat.lt_irrefl in H8; false.
...
Proof.
  ...
  unfold precise in p.
  pose proof p (e, e0, x5, i0, l, x8, a) x5 x x8 x2 H13 H8.
  pose proof inv_ieon_inv_ieoff_precise I.
  unfold precise in H0.
  pose proof H0 (e, e0, x6, i0, l, x9, a) x6 x0 x9 x3 H14 H9.

  assert(Maps.sub x5 M).
  { eapply sub_trans with M1;eauto.
  eapply join_sub_l; eauto. }
  assert(Maps.sub x M).
  { eapply sub_trans with M2;eauto.
  eapply join_sub_l; eauto. }
  assert(Maps.sub x8 o).
  { eapply sub_trans with o1;eauto.
  eapply join_sub_l; eauto. }
  assert(Maps.sub x2 o).
  { eapply sub_trans with o2;eauto.
  eapply join_sub_l; eauto. }
  specialize (H _ _ H11 H15 H16 H17).
  destruct H;subst.

  assert(Maps.sub x6 M).
  { eapply sub_trans with M1;eauto.
  eapply join_sub_r; eauto. }
  assert(Maps.sub x0 M).
  { eapply sub_trans with M2;eauto.
  eapply join_sub_r; eauto. }
  assert(Maps.sub x9 o).
  { eapply sub_trans with o1;eauto.
  eapply join_sub_r; eauto. }
  assert(Maps.sub x3 o).
  { eapply sub_trans with o2;eauto.
  eapply join_sub_r; eauto. }
  specialize (H6 _ _ H H18 H19 H20).
  destruct H6;subst.
  split; jeat.
Qed.
