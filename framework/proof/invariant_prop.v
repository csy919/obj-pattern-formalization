...
Proof.
  ...
  replace (j-0) with j in Hinv by lia.
  destruct j.
  inverts Hij.
  assert (i<= j) by lia.
  ...
  simpl getabst in *.
  simpl getmem in *.
  assert ( (S j - (i + 1) =  (S j - S i) ))%nat by lia.
  rewrite H0.
...
Proof.
  ...
  inversion H0;subst.
  assert (x0=t').
  eapply mapsto_load_eq; try eapply H12; eauto.
...
Lemma new_part_o:
  forall O Ol To t Oc Tl,
    ...
    partO Ol Tl To ->
    partO (merge O (@minus absdataid absdata osabst AMap Ol Oc)) Tl (TOSpecMod.put To t O).
Proof.
  intros.
  inductions H1.
  tryfalse.
...
Proof.
  ...
  inverts H27.
  assert (x0=t').
  eapply mapsto_load_eq; try eapply H8; eauto.
  subst x0.
  rewrite H in H3.
  inverts H3.
  apply H2.
...
Proof.
  ...
  clear- H5.
  unfold getinv in *.
  destruct (I (S INUM)) as (INV , prec , prec0).
  eapply good_inv_prop'; eauto.
  ...
  eapply join_merge_disj.
  eapply osabst_join_disjoint_disjoint; eauto.
  destruct (I (S INUM)) as (INV , goodi , goodi0).
  unfold getinv in *.
  eapply good_inv_prop'; eauto.
...
Proof.
  ...
  tryfalse.
  tryfalse.
  change (e, e0, M1, x, l, o1, a) with (substmo (e, e0, M1, x, l, o1, a) M1 o1) in H19.
  change (e, e0, M2, x, l, o2, a) with (substmo (e, e0, M1, x, l, o1, a) M2 o2) in H9.
  ...
  unfold sat in H, H0; fold sat in H, H0; fold cjy_and1; mytac.
  destruct s as [ [ [ [ [ [ ] ] ] ] ] ]; simpl in *.
  destruct H13, H8; mytac; unfold cjy_and1.
  change (e, e0, M1, x11, l, o1, a) with (substmo (e, e0, M1, x11, l, o1, a) M1 o1) in H14.
  change (e, e0, M2, x11, l, o2, a) with (substmo (e, e0, M1, x11, l, o1, a) M2 o2) in H9.
  lets H100 : IHn H14 H9; auto.
  eapply H100;eauto.

  tryfalse.
  tryfalse.

  change (e, e0, x6, x11, l, x9, a) with (substmo (e, e0, x6, x11, l, x9, a) x6 x9) in H14.
  change (e, e0, x0, x11, l, x3, a) with (substmo (e, e0, x6, x11, l, x9, a) x0 x3) in H9.
  lets H100 : IHn H14 H9.
  change (e, e0, x5, x11, l, x8, a) with (substmo (e, e0, x5, x11, l, x8, a) x5 x8) in H25.
  change (e, e0, x, x11, l, x2, a) with (substmo (e, e0, x5, x11, l, x8, a) x x2) in H15.
  assert (precise (getinv (I low))) as H200 by apply prec.
  lets H300 : H200 H25 H15; clear H200.
  
  assert (sub x6 M). 
  { eapply sub_trans with M1;eauto. eapply mem_join_sub_r;eauto. }
  assert (sub x0 M). 
  { eapply sub_trans with M2;eauto. eapply mem_join_sub_r;eauto. }
  assert (sub x9 o). 
  { eapply sub_trans with o1;eauto. eapply osabst_join_sub_r;eauto. }
  assert (sub x3 o). 
  { eapply sub_trans with o2;eauto. eapply osabst_join_sub_r;eauto. }
  specialize (H100 _ _ H H0 H6 H8). destruct H100.

  assert (sub x5 M). 
  { eapply sub_trans with M1;eauto. eapply mem_join_sub_l;eauto. }
  assert (sub x M). 
  { eapply sub_trans with M2;eauto. eapply mem_join_sub_l;eauto. }
  assert (sub x8 o). 
  { eapply sub_trans with o1;eauto. eapply osabst_join_sub_l;eauto. }
  assert (sub x2 o). 
  { eapply sub_trans with o2;eauto. eapply osabst_join_sub_l;eauto. }
  specialize (H300 _ _ H17 H18 H19 H20). destruct H300.
  subst.
...
Proof.
  ...
  assert ( (S j - (i + 1) =  (S j - S i) ))%nat by lia.
  rewrite H6 in H3.
  replace (i+1) with (S i) in H3 by lia.
  ...
  specialize (Hres _ _ H13 H14 H15 H16).
  destruct Hres as [Heq2 Heq1].
  subst.
...
Proof.
  ...
  specialize (Hres _ _ H14 H13 H16 H15).
  destruct Hres.
  subst x4; subst x7.
...
: forall (po pi : progunit) (ip : intunit) (A : osspec) 
    ...
    (o, O, ab (rev vl)) |= pf /\ satp o O (CurLINV lasrt tid).
Proof.
  intros.              
  ...
  simpl. 
  assert ((rev vl) = app (rev vl) nil).
  { rewrite <- app_nil_end; auto. }
  rewrite H5 at 1.
  eauto. (* added due to change in BuildPreA *)
...
(* the following change that was attempted due to changes in the def of BuildPreA is unecessary *)
(* Lemma bp_bpa *)
(* : forall (po pi : progunit) (ip : intunit) (A : osspec)  *)
(*          (o : taskst) (O : osabst) (d1 d2 : decllist)  *)
(*          (s : stmts) (vl : vallist) (pf : asrt) (t : type)  *)
...
(*          A_dom_lenv (getlenvdom (revlcons d1 d2))) -> *)
(*     get_genv (get_smem o) = G -> *)
(*     exists vl', (o, O, ab (app (rev vl) vl')) |= pf /\ satp o O (CurLINV lasrt tid). *)
(* Proof. *)
(*   intros.               *)
(*   unfold BuildPreA in H2. *)
(*   rewrite H0 in H2. *)
...
(*   exists (nil:list val). (* added due to change in BuildPreA *) *)

(*   split. *)
(*   split. *)
(*   simpl. *)
(*   auto. *)
(*   pose proof H4 (ab ((rev vl)++nil)%list); clear H4. *)
(*   sep auto. *)

(*   simpl. eauto. (* added due to change in BuildPreA *) *)
...
Definition subdomSO := 
  fun (S : osstate) (O : osabst) =>
    ...
    | Some (ostm _) => False
    | Some (oscurt _) => False
    | Some (absptls _) => False
    | Some (absobjs _) => False 
    | None => False
    end.
...
Proof.
  ...
  assert (
      (G, E, x0, isr, auxs, x2, aop)
        |= (EX tp : type, GV OSTCBCur @ Tptr tp |-r-> Vptr t) **
        init_rdy pa t
        (logic_val (Vint32 (Int.repr 1)) ::
                   logic_val (Vint32 (Int.repr __Loc_normal)) ::
(*                    logic_val Vnull :: 
                   logic_val Vnull ::  *)
                   logic_val Vnull :: nil)).  
  gen H7.
...
(*--*)
  ...
  assert ((logic_val (Vint32 (Int.repr 1)) :: logic_val (Vint32 (Int.repr __Loc_normal))
                     :: logic_val Vnull :: (* logic_val Vnull :: logic_val Vnull ::  *)nil)%list
          = init_lg). 
