...
| HETr_abt : forall (p:hprog) (cst:clientst) (T:tasks)  (O:osabst),
    hpstepabt p T cst O -> HETr (p, T, cst, O) evabort
| HETr_absabt : forall (p:hprog) (cst:clientst) (T:tasks)  (O:osabst),
    prgabsabt p T cst O -> HETr (p, T, cst, O) evabsabt.
(* absabt T -> HETr (p, T, cst, O) evabsabt.
    mismatch between trace inclusion and program simulation ->
    failure with proof for progsim_imply_eref_aux *) 
...
Definition etrace_subset (lw: lworld)(hw: hworld)  := 
  forall E, LETr lw E -> ~HETr hw evabsabt -> HETr hw E. 
...
Lemma progsim_imply_eref_aux : 
  forall  (E : evtrace) (lp: lprog) (lt : tasks) (cs : clientst) (os : osstate) (t: tid),
    LETr (lp, lt, cs, os, t) E ->
    (forall (hp: hprog) (ht: tasks) (bs : osabst) ,
        ProgSim lp lt os  hp ht bs cs t ->
        ~(prgabsabt hp ht cs bs) -> 
        HETr (hp, ht, cs, bs) E).
Proof.
introv HLe.
inductions HLe.
...
eauto.
eauto.
eapply prgnabt_hpstepstar; eauto.

(* introv Hsim. *)
introv Hsim Hnabt.
inverts keep Hsim.
(* lets (Th' & O' & Hstep & Hpsim) : H2 H. *)
lets (Th' & O' & Hstep & Hpsim) : H2 H Hnabt. 
eapply  Hstepstar_with_ev.
eauto.
eapply IHHLe.
eauto.
eauto.
eapply prgnabt_hpstepevstar; eauto.
...
Proof.
  ...
  (* introv Hle. *)
  introv Hle Hnabt.
  assert (~ prgabsabt (P,T) CS OS0 t'). 
  { intro Hf. apply Hnabt. constructors; auto. }
  simpls. 
