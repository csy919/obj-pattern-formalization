...
Require Import tskabsabt_lemmas.
...
Lemma tsk_spec_nabt_impl0:
  forall pc A aop ke ks cst Of OO OOO t, 
    join Of OO OOO -> 
    ~ tskabsabt (pc, A) (curs (hapi_code aop), (ke, ks)) cst OOO t ->
    ~ (exists OO', hmstepstar (snd A) aop OO (ABORT) OO'). 
Proof. 
  introv Hjo Htsknabt Hex.
  apply Htsknabt.
  destruct Hex as (OO' & Hsp).
  eapply spec_stepstar_locality in Hsp; eauto.
  destruct Hsp as (O2 & Hsp' & Hjo'). 
  eapply absabt_tskabsabt; eauto.
Qed.
  
Lemma tsk_spec_nabt_impl:  
  forall Oi Ol OO Os OO0 pc A ab ke ks cst t, 
    join Oi Ol OO -> 
    join OO Os OO0 -> 
    ~tskabsabt (pc, A) (curs (hapi_code ab), (ke, ks)) cst OO0 t  ->
    ~(exists OO', hmstepstar (snd A) ab (merge Os Oi) (ABORT) OO').
Proof. 
  introv Hjo1 Hjo2 Hntsk.
  introv Hnex.
  apply Hntsk.
  assert (join Ol (merge Os Oi) OO0).  
  { 
    assert (Hdisj: disjoint Os OO). 
    { apply join_disj with (M:=OO0). apply join_comm. auto. }
    asserts_rewrite (merge Os Oi = merge Oi Os).
    {
      apply disjoint_merge_sym. 
      apply disjoint_sym in Hdisj. apply disjoint_sym.
      lets Hdis: perm_map_lemmas_part2.osabst_join_disjoint_disjoint Hjo1 Hdisj.
      auto.
    }
    apply join_comm.
...
Lemma tsk_spec_nabt_impl':  
  forall Oi Ol OO Os OO0 pc A ab ke ks cst t, 
    ...
    join OO Os OO0 -> 
    ~tskabsabt (pc, A) (curs (hapi_code ab), (ke, ks)) cst OO0 t  ->
    ~(exists OO', hmstepstar (snd A) ab (merge Oi Os) (ABORT) OO').
Proof.
  introv Hjo1 Hjo2 Hntsk.
  introv Hnex.
  apply Hntsk.
  assert (join Ol (merge Oi Os) OO0).  
  { 
    apply join_comm.
    eapply join_join_join_merge'; eauto.
    apply join_comm; auto.
...
Lemma prg_nabt_impl0:
  forall hp Th t Ch cst O Of OO,
    join O Of OO ->
    get Th t = Some Ch -> 
    get OO curtid = Some (oscurt t) -> 
    ~ prgabsabt hp Th cst OO -> 
    ~ tskabsabt hp Ch cst O t.
Proof.
  introv Hjo HTht Hctid Hnpabt.
  introv Hntabt. apply Hnpabt.
  eapply tskabsabt_prgabsabt; eauto.
  unfold tskabsabt in Hntabt.
  unfold tskabsabt.
  destruct Hntabt as (ke' & ks' & cst' & O' & Hhtsteps).
  exists ke' ks' cst'.
  lets H'': htsteps_local Hhtsteps Hjo.
  destruct H'' as (OO' & Hhtsteps' & Hjo').
  eexists; eauto.
Qed.    
...
Lemma IntSeq'': 
  forall pc (po:progunit) pi ip A p c ke ks t I r
    ...
    (forall Mlinv Olinv,
        satp ((ge,lenv,Mlinv),(isrupd isrreg i false),(true,si,nil)) Olinv (p_local lasrt t lg) ->
        TaskSim
          (pc,(po,pi,ip)) (c,(ke,ks)) ((ge,lenv,(merge Ml Mlinv)),(isrupd isrreg i false),(true,si,nil))
          (pc,A) (ch,(keh,ksh)) (merge Ol Olinv) lasrt I p t
    ) ->
    ...
    TaskSim
      (pc,(po,pi,ip)) (c',(ke', ks'## kint c ke lenv ks)) o
      (pc,A) (curs (hapi_code ab),(kenil,kevent ch keh ksh)) OO
      lasrt I p t. 
Proof.
  cofix CIH.
  introv Hnocallos.
  ...
  apply task_sim.
  intros.
  rename H10 into Htsknabt. (* added due to consideration of abstract abortion *)
  ...
  lets Hmsim0: H14 Hc Hcallcont Hintcont H18; eauto. 
  unfold getmem.
  simpl.
  clear -Hf3 Hf5.
  unfolds;join auto.
  (* added due to treatment of abstract abortion *) 
  assert (Hnabt: ~ (exists OO', hmstepstar (snd A) ab x (ABORT) OO')).
  {
    lets Hxmg: (join_merge H18). subst x.
    eapply tsk_spec_nabt_impl0; eauto.
    join auto.
  }
  lets Hmsim: Hmsim0 Hnabt.
  ...
  (* ~ (exists OO', hmstepstar (snd A) ab (merge Os Oi) (ABORT) OO') *)
  eapply tsk_spec_nabt_impl; eauto.  
  ...
  intros.
  rename H10 into Htsknabt. (* added due to consideration of abstract abortion *) 
  destruct Htsknabt as (cst111 & Htsknabt).
  ...
  intros.
  rename H15 into Htsknabt. (* added due to consideration of abstract abortion *) 
  inversion H5;subst.
  ...
  lets Habt0: H21 H10 H1; eauto.  
  Focus 2.
  
  assert (HdisjOis: disjoint Oi Os).
  {
    assert (Hdisj: disjoint OO Os) by (apply join_disj with (M:=OO0); auto).
    eapply join_disjoint_disjoint; eauto.
  }
  lets Habt00: Habt0 HdisjOis. clear Habt0.
  lets Hjo_is: (join_merge_disj HdisjOis). 
  lets Habt0: Habt00 Hjo_is. clear Habt00.
  assert (Hnex: ~(exists OO', hmstepstar (snd A) ab (merge Oi Os) (ABORT) OO')). 
  { 
    assert (Hjofrm: join (merge Ol Of) (merge Oi Os) OOO).
    {
      apply join_comm. 
      eapply join_merge_merge_recompose; eauto.
    }    
  ...
  lets Habt: Habt0 Hnex. 
  clear Habt0. clear Hnex Hjo_is HdisjOis Htsknabt.
  ...
  lets XX0: H17 X H10  H11 H24;eauto. 
  assert (Hnex: ~ (exists OO', hmstepstar (snd A) ab x (ABORT) OO')).
  {
    asserts_rewrite(x = merge Oi Os).
    { apply join_merge; auto. }
    assert (Hjofrm: join (merge Ol Of) (merge Oi Os) OOO).
    {
      apply join_comm. 
      eapply join_merge_merge_recompose; eauto.
  ...
  lets Hx0: H18 X H10  H11 H24;eauto.
  assert (Hnex: ~ (exists OO', hmstepstar (snd A) ab x (ABORT) OO')).
  {
    asserts_rewrite(x = merge Oi Os).
    { apply join_merge; auto. }
  ...
  lets Hx0: H19 X H10  H11 H24;eauto.
  assert (Hnex: ~ (exists OO', hmstepstar (snd A) ab x (ABORT) OO')).
  {
    asserts_rewrite(x = merge Oi Os).
    { apply join_merge; auto. }
  ...
  join auto.
  destruct H24.
  (* lets Hx0: H20 X XXX H11 H24;eauto.  *)
  lets Hx0: H20 X XXX H11 H24;eauto. 
  assert (Hnex: ~ (exists OO', hmstepstar (snd A) ab x (ABORT) OO')).
  {
    asserts_rewrite(x = merge Oi Os).
...
Proof.
  ...
  apply task_sim.
  intros.
  rename H11 into Hntskabt. (* added due to change in def of task sim *) 
  assert (o2=o'').
...
(*  eapply ltstep_good_ret;eauto.*)
  ...
  intros.
  rename H11 into Hntskabt. (* added due to change in def of task sim *) 
  destruct Hntskabt as (cst111 & Hntskabt).
  ...
  apply Hinit;auto.
  intros.
  rename H16 into Hntskabt. (* added due to change in def of task sim *) 
  unfold IsEnd in H7.
  apply htabtstar.
...
Ltac mytac_2 :=
    match goal with
      | H : exists _, _ |- _ => destruct H; mytac_2
      | H : (_, _) = (_, _) |- _ => inversion H; clear H; mytac_2
      | _ => idtac
    end.
  ...
  apply task_sim;intros;
    try (match goal with 
           [H: (~ tskabsabt _ _ _ _ _) |- _] => rename H into Hntskabt
         | [H: (~exists cst, tskabsabt _ _ _ _ _) |- _] => rename H into Hntskabt
         end);
    tryfalse.
  (* rename H10 into Hntskabt. (* added due to change in def of task sim *)  *)
  ...
  (*-------------------------------*)
  destruct Htlmatch as [Htlmatch H1].
  apply task_sim.
  intros.
  rename H11 into Hntskabt. (* added due to change in def of task sim *) 
  inversion H10;tryfalse.
  ...
  (* (* the following two lines are added due to changes in BuildPreA *)  *)
  (* rename Hpre into Hpre0. *)
  (* destruct Hpre0 as (vl'&Hpre). *)
  ...
  subst cst'.
  assert (Hnex: ~ (exists OO', hmstepstar (snd (B, C, sc)) AC OO (ABORT) OO')).
  {
    eapply tsk_spec_nabt_impl0 with (Of:=emp)(OOO:=OO); eauto. 
    join auto.
  }
  lets Hmsim':H13 H7 H8 H9 Hlosstep Hnex;eauto.
  ...
  (* lets Hmsim'0: H16 Hcallcont Hintcont H9;eauto. *)
  lets Hmsim'0: H16 Hcallcont Hintcont H9;eauto.
  ...
  lets Hmsim': Hmsim'0 Hnex. clear Hmsim'0.
  ...
  lets Hmsim'0: H14 Hcallcont Hintcont H9;eauto. 
  clear -H8.
  unfolds.
  unfold joinm2 in H8.
  ...
  (* ~ (exists OO', hmstepstar (snd (B, C, sc)) AC OO (ABORT) OO') *)
  eapply tsk_spec_nabt_impl0 with (Of:=emp)(OOO:=OO); eauto. 
  join auto.
  ...
  lets Habt0: H22 H11 H12 H25;eauto. 
  Focus 2.

  assert (Hjo_O_Os: join O Os (merge O Os))
           by (apply join_merge_disj; auto).
  lets Habt00: Habt0 Hjo_O_Os. clear Habt0.
  assert (Hnex: ~ (exists OO', hmstepstar (snd (B, C, sc)) AC (merge O Os) (ABORT) OO')).
  {
    eapply tsk_spec_nabt_impl0; eauto.
    join auto.
  }
  lets Habt: Habt00 Hnex. clear Habt00. 
  ...
  lets XX: H18 X H11 H26 H13 Hnex;eauto. 
  ...
  lets XXX0:H21 X H28 H27 H26 H13;eauto.
  assert (Hnex: ~ (exists OO', hmstepstar (snd (B, C, sc)) AC OO (ABORT) OO')).
  {
    eapply tsk_spec_nabt_impl0; eauto. 
    join auto.
  }
  lets XXX: XXX0 Hnex. clear XXX0.
  ...
  lets Hx:H17 H8 H9 H10 Hnex;eauto.
  ...
  lets Hx:H18 H8 H9 H10 Hnex;eauto. 
  ...
  inversion H1.
  intros.
  rename H15 into Hntskabt. (* added due to change in def of task sim *) 
...
Proof.
  ...
  auto.
  intros. 
  rename H0 into Hnprgabt. (* added due to change in def of task sim *) 
  subst.
  ...
  (* ~ tskabsabt (pc, A) Ch cst (merge Oc Os) t' *)
  (* Search (partO _ _ _). *)
  assert (H_sub_c_l: sub Oc Ol).
  { eapply parto_sub; eauto. }
  unfold sub in H_sub_c_l.
  destruct H_sub_c_l as (Of00 & Hjofrm).      
  eapply prg_nabt_impl0; eauto.
  instantiate (1:=Of00).
  apply join_comm. 
...
(* ** ac:   Check  store_sub_minus_eq. *)
  ...
  *)
  ...
  (* exists cst, ~ tskabsabt (pc, A) Ch cst (merge Oc Os) t *)
  assert (H_sub_c_l: sub Oc Ol). 
  { eapply parto_sub; eauto. }
  unfold sub in H_sub_c_l.
  destruct H_sub_c_l as (Of00 & Hjofrm).
...
*)
  ...
  (* ~ tskabsabt (pc, A) Ch cst O t *)
  { eapply prg_nabt_impl0; eauto. join auto. }
  ...
  (* exists cst0, ~ tskabsabt ph Ch cst0 (merge Oc Os) t *) 
  assert (H_sub_c_l: sub Oc Ol). 
  { eapply parto_sub; eauto. }
  unfold sub in H_sub_c_l.
  destruct H_sub_c_l as (Of00 & Hjofrm).
