...
Module idxspec.
  ...
  Definition beq := Int.eq.
  Definition blt := Int.lt.
  Lemma beq_true_eq : 
    forall a a' : A,
      beq a a' = true -> a = a'.
  Proof.
    ...
    assert (forall a a' : int32, if Int.eq a a' then a = a' else a <> a').
    apply Int.eq_spec.
    intros.
    specialize (H a a').
    rewrite H0 in H.
  ...
  Proof.  
    unfold blt; intros.
    unfold Int.lt.
    destruct (zlt (Int.signed a) (Int.signed a)).
    omega.
    auto.
  Qed.
  ...
  Proof.  
    ...
    destruct ( zlt (Int.signed b) (Int.signed a));
      destruct ( zlt (Int.signed a) (Int.signed b)).
    omega.
    auto.
    auto.
    auto.
  ...
  Proof.
    (*
    intros.
    remember (blt a b) as bool1. destruct bool1.
    left; left; auto.
    remember (beq a b) as bool2. destruct bool2.
    left; right; auto.
    right.


    symmetry in Heqbool1; apply Nat.ltb_ge in Heqbool1.
    symmetry in Heqbool2; apply Nat.eqb_neq in Heqbool2.
    assert (b < a) as H100 by omega.
    apply Nat.ltb_lt in H100.
    right; auto.
  Qed.
     *)
    unfold blt; unfold beq; intros.
    unfold Int.lt.
    unfold Int.eq.
    remember ( zlt (Int.signed a)(Int.signed b)) as H1.
    remember ( zeq (Int.unsigned a)(Int.unsigned b)) as H2.
    remember ( zlt (Int.signed b)(Int.signed a)) as H3.
    destruct H1;destruct H2;destruct H3;auto.
    unfold Int.unsigned in *.
    simpl in *.
    unfold Int.signed in *.
